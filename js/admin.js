...
(function ($) {
  ...
  $.fn.miniTip = function (t) {
    ...
    admin &&
      ...
      admin.append(
        '<div id="miniTip" class="sb-tooltip"><div></div></div>'
      );
  ...
  $.fn.sbLanguageSwitcher = function (
    ...
    let added = [];
    let element = $(this).hasClass(
      "sb-language-switcher-cnt"
    )
      ? $(this)
      : $(this).find(".sb-language-switcher-cnt");
    for (var i = 0; i < langs.length; i++) {
      let language = isString(langs[i])
        ? langs[i]
        : langs[i][0];
      let id =
        isString(langs[i]) || !langs[i][1]
          ? false
          : langs[i][1];
  ...
  function editorJSHTML(blocks) {
    ...
    blocks.map((block) => {
      switch (block.type) {
        ...
        case "list":
          code +=
            '<ul class="sb-ul-' + block.data.style + '">';
  ...
  function saved_reply_search(search) {
    SBF.search(search, () => {
      ...
      if (
        !all &&
        (SB_ADMIN_SETTINGS.dialogflow ||
          SB_ADMIN_SETTINGS.chatbot_features)
      ) {
        let icon = area
          .closest(".sb-popup")
          .find(".sb-icon-search");
        ...
        if (SB_ADMIN_SETTINGS.dialogflow) {
          SBApps.dialogflow.getIntents((response) => {
            let intents = SBApps.dialogflow.searchIntents(
              search,
              true
            );
            let code_2 = "";
          ...
          SBF.ajax(
            ...
            (response) => {
              area.html(
                response[1] &&
                  (!response[5] ||
                    !response[5].unknow_answer)
                  ? `${code}<li><div></div><div>${response[1]}</div></li>`
                  : code
                  ? code
                  : no_results
              );
  ...
  function whatsapp_direct_message_box(user_ids) {
    ...
    SBF.ajax(
      ...
      (response) => {
        ...
        for (var i = 0; i < response.length; i++) {
          if (
            ...
            (!SB_ACTIVE_AGENT.department ||
              !response[i].department.length ||
              response[i].department.includes(
                SB_ACTIVE_AGENT.department
              ))
          ) {
        ...
        box
          .find("#sb-whatsapp-send-template-list")
          .html(code);
        SBForm.clear(box);
        box
          .find(".sb-direct-message-users")
          .val(
            user_ids.length ? user_ids.join(",") : "all"
          );
  ...
  var SBApps = {
    dialogflow: {
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            if (response.dialogflow_languages) {
              this.dialogflow_languages =
                response.dialogflow_languages;
            }
          }
        );
      },
      ...
      showCreateIntentBox: function (message_id) {
        let question = "";
        let message =
          SBChat.conversation.getMessage(message_id);
        let response = message.message;
        if (SBF.isAgent(message.get("user_type"))) {
          question = SBChat.conversation.getLastUserMessage(
            message.get("index")
          );
          if (
            question &&
            question.payload("sb-human-takeover")
          ) {
            question =
          ...
          question = response;
          let agent_message =
            SBChat.conversation.getNextMessage(
              message.id,
              "agent"
            );
          if (agent_message) {
        ...
        if (
          dialogflow_intent_box.hasClass(
        ...
        dialogflow_intent_box
          ...
          .remove();
        dialogflow_intent_box
          .find(".sb-type-text input")
          .val(question);
        dialogflow_intent_box
          .find("#sb-intents-select,#sb-qea-select")
          .val("");
        dialogflow_intent_box
          .find(".sb-search-btn")
          .sbActive(false)
      ...
      submitIntent: function (button) {
        ...
        let questions = [];
        let answer = dialogflow_intent_box
          .find("textarea")
          .val();
        let intent_name = dialogflow_intent_box
          .find("#sb-intents-select,#sb-qea-select")
          .val();
        let services = dialogflow_intent_box
        ...
        if (
          ...
          if (is_open_ai) {
            if (intent_name) {
              this.qea[intent_name][0] =
                this.qea[intent_name][0].concat(questions);
          ...
          SBF.ajax(
            {
              ...
              services: services,
              language: dialogflow_intent_box
                .find(".sb-dialogflow-languages select")
                .val(),
            },
            (response) => {
      ...
      getIntents: function (onSuccess) {
        if (this.intents === false) {
          SBF.ajax(
            ...
            (response) => {
              this.intents = Array.isArray(response)
                ? response
                : [];
      ...
      searchIntents: function (
        search,
        return_intents = false
      ) {
        if (
          dialogflow_intent_box.hasClass(
            "sb-dialogflow-disabled"
          ...
          let all = search.length > 1 ? false : true;
          let code = all
            ? `<option value="">${sb_("New Q&A")}</option>`
            : "";
          let list = [];
          ...
          for (var i = 0; i < this.qea.length; i++) {
            if (
              ...
              this.qea[i][0]
                ...
                .toLowerCase()
                .includes(search) ||
              this.qea[i][1].toLowerCase().includes(search)
            ) {
          ...
          for (var i = 0; i < intents.length; i++) {
            let found =
              all ||
              intents[i].displayName
                .toLowerCase()
                .includes(search);
            if (!found && intents[i].trainingPhrases) {
              let training_phrases =
                intents[i].trainingPhrases;
              for (
                var j = 0;
                j < training_phrases.length;
                j++
              ) {
                for (
                  var y = 0;
                  y < training_phrases[j].parts.length;
                  y++
                ) {
                  if (
      ...
      previewIntent: function (index) {
        if (index) {
          ...
          if (qea[0].length > 1) {
            let max =
              qea[0].length > 15 ? 15 : qea[0].length;
            for (var i = 0; i < max; i++) {
    ...
    openAI: {
      ...
      rewrite: function (message, onSuccess) {
        SBF.ajax(
          ...
          (response) => {
            if (!response[0]) {
              console.error(
                "OpenAI: " + JSON.stringify(response[1])
              );
            }
      ...
      troubleshoot: function () {
        let status =
          SB_ADMIN_SETTINGS.open_ai_chatbot_status;
        if (status !== true) {
          infoBottom(
            status == "inactive"
      ...
      getCode: {
        set_data: function (data) {
          let code = "";
          let code_select_user_details =
            this.select_user_details();
          if (!data || !data.length) {
            data = [["", ""]];
          }
          for (var i = 0; i < data.length; i++) {
            code += `<div class="repeater-item"><div>${code_select_user_details.replace(
              `"${data[i][0]}"`,
              `"${data[i][0]}" selected`
            )}<div class="sb-setting"><input type="url" placeholder="${sb_(
              "Enter the value"
            )}" value="${
              data[i][1]
            }"></div></div><i class="sb-icon-close"></i></div>`;
          }
        ...
        actions: function (actions) {
          let action_list = [
            ...
            ["redirect", "Go to URL"],
            ["open_article", "Show an article"],
            ["transcript", "Download transcript"],
            ["transcript_email", "Email transcript"],
            ["send_email", "Send email to user"],
            ["send_email_agents", "Send email to agents"],
            [
          ...
          for (var i = 0; i < action_list.length; i++) {
            code += `<option value="${
              action_list[i][0]
            }">${sb_(action_list[i][1])}</option>`;
          }
          for (var i = 0; i < actions.length; i++) {
            code_2 += `<div class="repeater-item"><div><div class="sb-setting"><select>${code.replace(
              `"${actions[i][0]}"`,
              `"${actions[i][0]}" selected`
            )}</select></div>${this.action(
              actions[i][0],
              actions[i][1]
            )}</div><i class="sb-icon-close"></i></div>`;
          }
        ...
        action: function (action, value) {
          let help = {
            ...
            department: "Enter the department ID",
            agent: "Enter the agent ID",
            redirect: "Enter the URL",
            open_article: "Enter the article ID",
            send_email: "Enter a message",
            send_email_agents: "Enter a message",
          };
          let input_types = {
            department: "number",
            agent: "number",
          ...
          return [
            ...
            ? `<div class="sb-setting"><input type="${
                input_types[action]
                  ? input_types[action]
                  : "text"
              }" value="${value}" placeholder="${sb_(
                help[action]
              )}" value="${value}"></div>`
            : "";
        },
        ...
        select_user_details: function () {
          ...
          let user_details = [
            ...
            ["email", "Email"],
            ["password", "Password"],
          ].concat(SBUsers.getExtraDetailsList());
          for (var i = 0; i < user_details.length; i++) {
            code += `<option value="${
              user_details[i][0]
            }">${sb_(user_details[i][1])}</option>`;
          }
          return code + `</select></div>`;
        },

        repeater_: function (title, code) {
          return `<div class="sb-title">${sb_(
            title
          )}</div><div data-type="repeater" class="sb-setting sb-type-repeater"><div class="input"><div class="sb-repeater sb-repeater-block-${SBF.stringToSlug(
            title
          )}">${code}</div><div class="sb-btn sb-btn-white sb-repeater-add sb-icon"><i class="sb-icon-plus"></i>${sb_(
            "Add new item"
          )}</div></div></div>`;
        },
      },
      ...
      generateQuestions: function (expression) {
        if (
          SB_ADMIN_SETTINGS.open_ai_user_expressions &&
          expression
        ) {
          let loader = dialogflow_intent_box.find(
            '[data-value="add"]'
          );
          loader.sbLoading(true);
          dialogflow_intent_box
            .find(".sb-open-ai-intent")
            .remove();
          SBF.ajax(
            {
              function: "open-ai-user-expressions",
              message: expression,
            },
            (response) => {
              let code = "";
              for (var i = 0; i < response.length; i++) {
                if (response[i]) {
                  code += `<div class="sb-setting sb-type-text sb-open-ai-intent"><input type="text" value="${response[
                    i
                  ].replace(/"/g, "")}"></div>`;
                }
              }
      ...
      flows: {
            ...
            for (var i = 0; i < flow.steps.length; i++) {
              ...
              for (var j = 0; j < block_cnts.length; j++) {
                ...
                for (var y = 0; y < blocks.length; y++) {
                  if (
                    blocks[y].type == "start" &&
                    !Array.isArray(blocks[y].message)
                  )
                    blocks[y].message = [
                      { message: blocks[y].message },
                    ]; // Deprecated
                  let text = blocks[y].message
                    ? Array.isArray(blocks[y].message)
                      ? blocks[y].message.length
                        ? blocks[y].message[0].message
                        : ""
                      : blocks[y].message
                    : "";
                  if (text) {
                  ...
                  code += `<div class="sb-flow-block" data-type="${
                    blocks[y].type
                  }"><div>${sb_(
                    SBF.slugToString(blocks[y].type)
                  )}</div>${text}`;
                  switch (blocks[y].type) {
                    ...
                    case "button_list":
                      let is_condition =
                        blocks[y].type == "condition";
                      let rows = is_condition
                        ? [sb_("True"), sb_("False")]
                        : blocks[y].options;
                      if (is_condition) {
                        code += `<div>`;
                      ...
                      for (
                        ...
                        labels.push(
                          letters[letters_index] + (x + 1)
                        );
                        code += `<div>${rows[x]}<span>${
                ...
                if (blocks.length < 4) {
                  code +=
                    '<div class="sb-flow-add-block sb-icon-plus"></div>';
                }
                code += "</div>";
              }
              code +=
                '</div><div class="sb-flow-add-step sb-icon-plus"></div></div>';
            }
            flows_area.html(code);
            $(admin)
              .find(".sb-flow-scroll")
              .sbActive(
                flow.steps.length * 250 >
                  flows_area.outerWidth()
              );
          }
        },
        ...
        navCode: function (name, active = false) {
          ...
          } data-value="${name}">${name}<i class="sb-icon-delete"></i></li>`;
        },
        ...
        steps: {
          get: function (
            ...
            return SBApps.openAI.flows.get(flow_name).steps[
              step_index
                ? step_index
                : this.getActiveIndex()
            ];
          ...
          getActiveIndex: function () {
            return SBApps.openAI.flows.blocks
              .getActive()
              .parent()
              .parent()
              .parent()
        ...
        blocks: {
          set: function (
            ...
            if (flow) {
              ...
              let next_step =
                flow.steps[
                  SBApps.openAI.flows.steps.getActiveIndex() +
                    1
                ];
                ...
                for (var j = 0; j < step[i].length; j++) {
                  switch (step[i][j].type) {
                    case "get_user_details":
                      ...
                      break;
                    case "button_list":
                      next_step_block_cnts_count +=
                        step[i][j].options.length;
                      break;
                    case "condition":
              ...
              if (next_step) {
                if (
                  ...
                  next_step.splice(
                    (next_step_block_cnts_count - 1) * -1,
                    next_step_block_cnts_count
                  );
                } else {
          ...
          get: function (
            ...
            if (flow) {
              ...
              return flow.steps[indexes.step][indexes.cnt][
                indexes.block
              ];
            }
            return false;
          ...
          add: function (
            ...
            this.set(
              Object.assign(attributes[block_type], {
                type: block_type,
              }),
              indexes.name,
              indexes.step,
          ...
          delete: function (
            ...
            for (
              var i = 0;
              i < SBApps.openAI.flows.flows.length;
              i++
            ) {
              ...
              if (indexes.name == flow.name) {
                if (
                  [
                    ...
                    flow.steps[indexes.step][indexes.cnt][
                      indexes.block
                    ].type
                  )
                ) {
                  let block_cnts_to_delete = this.delete_(
                    i,
                    indexes.step,
                    indexes.cnt
                  );
                  ...
                  for (
                    var j = 0;
                    j < block_cnts_to_delete.length;
                    j++
                  ) {
                    flow.steps[block_cnts_to_delete[j][0]][
                      block_cnts_to_delete[j][1]
                    ] = false;
                  }
                  flow.steps[indexes.step][
                    indexes.cnt
                  ].splice(indexes.block, 1);
                  flow_new.steps = [];
                  for (
                    var j = 0;
                    j < flow.steps.length;
          ...
          delete_: function (
            ...
            step_index,
            block_cnt_index,
            all_next_block_cnt_indexes = []
          ) {
            let next_block_cnt_indexes =
              SBApps.openAI.flows.blocks.getNextCntIndexes(
                i,
                step_index,
                block_cnt_index
              );
            for (
              var j = 0;
              j < next_block_cnt_indexes.length;
              j++
            ) {
              all_next_block_cnt_indexes.push([
                step_index + 1,
                next_block_cnt_indexes[j],
              ]);
              this.delete_(
                i,
                step_index + 1,
          ...
          getActive: function () {
            return flows_area.find(
              ".sb-flow-add-block.sb-active, .sb-flow-block.sb-active"
            );
          },

          getActiveIndex: function () {
            let blocks = this.getActiveCnt().find(
              ".sb-flow-block"
            );
          ...
          getNextCnt: function (
            current_flow_index,
            current_step_index,
            current_block_cnts_index,
            current_connector_index = 0
          ) {
            let indexes = this.getNextCntIndexes(
              current_flow_index,
              current_step_index,
              current_block_cnts_index,
              current_connector_index
            );
            return indexes.length > current_connector_index
              ? SBApps.openAI.flows.flows[
                  current_flow_index
                ].steps[current_step_index + 1][
                  indexes[current_connector_index]
                ]
              : false;
          },

          getNextCntIndexes: function (
            current_flow_index,
            current_step_index,
            current_block_cnt_index
          ) {
            let flow =
              SBApps.openAI.flows.flows[current_flow_index];
            let next_block_cnt_indexees = [];
            if (
              flow &&
              flow.steps[current_step_index + 1]
            ) {
              let next_block_cnt_index = 0;
              for (
                var i = 0;
                i <= current_block_cnt_index;
                i++
              ) {
                let current_blocks =
                  flow.steps[current_step_index][i];
                for (
                  var j = 0;
                  j < current_blocks.length;
                  j++
                ) {
                  if (
                    current_blocks[j].type == "button_list"
                  ) {
                    for (
                      var k = 0;
                      k < current_blocks[j].options.length;
                      k++
                    ) {
                      if (i == current_block_cnt_index) {
                        next_block_cnt_indexees.push(
                    ...
                    current_blocks[j].type == "condition"
                  ) {
                    for (var k = 0; k < 2; k++) {
                      if (i == current_block_cnt_index) {
                        next_block_cnt_indexees.push(
          ...
          getPreviousCntIndex: function (
            ...
            if (previous_step) {
              ...
              for (
                var i = 0;
                i < previous_step.length;
                i++
              ) {
                let block_cnt = previous_step[i];
                for (var j = 0; j < block_cnt.length; j++) {
                  index +=
                    block_cnt[j].type == "button_list"
                      ? block_cnt[j].options.length
                      : block_cnt[j].type ==
                        "get_user_details"
                      ? 1
                      : block_cnt[j].type == "condition"
                      ? 2
                      : 0;
                }
          ...
          getIndexes: function (
            ...
            return {
              name: flow_name
                ? flow_name
                : SBApps.openAI.flows.getActiveName(),
              step: step_index
                ? step_index
                : SBApps.openAI.flows.steps.getActiveIndex(),
              cnt: block_cnt_index
                ? block_cnt_index
                : this.getActiveCntIndex(),
              block: block_index
                ? block_index
                : this.getActiveIndex(),
            };
          ...
          activateLinkedCnts: function (active_block) {
            ...
            let active_flow_index =
              SBApps.openAI.flows.getActiveIndex();
            let active_step_index = block_cnt
              .parent()
              .parent()
              .index();
            let flows = flows_area.find("> div");
            let previous_cnt = flows
              .eq(active_step_index - 1)
              .find(".sb-flow-block-cnt")
              .eq(
                SBApps.openAI.flows.blocks.getPreviousCntIndex(
                  active_flow_index,
                  active_step_index,
                  block_cnt.index()
                )
            ...
            if (!is_over_connector) {
              ...
              for (
                var i = 0;
                i < next_block_cnt_indexes.length;
                i++
              ) {
                next_block_cnts
                  .eq(next_block_cnt_indexes[i])
                  .sbActive(true);
              }
            }
            flows_area
              .find(".sb-flow-connectors > div")
              .sbActive(false);
            previous_cnt
              .sbActive(true)
              .find(".sb-flow-block-cnt-name")
              .sbActive(true);
            if (
              block_cnt.find(".sb-flow-block-cnt-name")
                .length
            ) {
              previous_cnt
                .find(".sb-flow-connectors > div")
      ...
      train: {
        ...
        files: function (onSuccess, index = 0) {
          ...
          if (
            ...
            !this.skip_files.includes(file_name)
          ) {
            admin
            ...
            upload_input.sbUploadFiles((response) => {
              ...
              if (response[0] == "success") {
                SBF.ajax(
                  {
                    function: "open-ai-file-training",
                    url: response[1],
                  },
                  (response) => {
              ...
              SBF.ajax(
                ...
                (response) => {
                  if (this.isError(response)) {
                    ...
                    !response[1].includes("http-error-302")
                  ) {
                    if (
                      response[1].includes("http-error") &&
                      index === 0
                    ) {
        ...
        sitemap: function (
          ...
          SBF.ajax(
            { function: "generate-sitemap", url: url },
            (response) => {
              onSuccess(response);
            }
          );
        ...
        qea: function (onSuccess) {
          admin
            .find("#sb-embeddings-box p")
            .html(sb_("We are processing the Q&A."));
          let questions_answers = SBSettings.repeater
            .get(
              chatbot_qea_repeater
                .find(".sb-repeater")
                .eq(0)
                .find("> .repeater-item")
            )
            .map((item) => {
              return [
                item["open-ai-faq-questions"].map(
                  (item) => {
                ...
                item[
                  "open-ai-faq-function-calling-headers"
                ],
                item[
                  "open-ai-faq-function-calling-properties"
                ].length
                  ? item[
                      "open-ai-faq-function-calling-properties"
                    ].map((item) => {
        ...
        articles: function (onSuccess) {
          ...
          SBF.ajax(
            { function: "open-ai-articles-training" },
            (response) => {
              onSuccess(response);
            }
          );
        ...
        isFile: function (url) {
          return (
            url.includes(".pdf") || url.includes(".txt")
          );
        },
      ...
      playground: {
        ...
        addMessage: function (
          ...
          chatbot_playground_area.append(
            `<div data-type="${user_type}"><div>${sb_(
              user_type == "user" ? "User" : "Assistant"
            )}<div><i class="sb-icon-close sb-btn-icon sb-btn-red"></i></div></div><div>${new SBMessage(
              {
                id: 1,
          ...
          chatbot_playground_area[0].scrollTop =
            chatbot_playground_area[0].scrollHeight;
          ...
          (response) => {
            ...
            for (var i = 0; i < response.length; i++) {
              if (
                ...
                let is_file = this.train.isFile(
                  response[i]
                );
                code[is_file ? 1 : 0] += `<tr data-url="${
                  response[i]
                }"><td><input type="checkbox" /></td><td>${
                  is_file
                    ? SBF.beautifyAttachmentName(
                        response[i].split("/").pop()
                      )
                    : urlStrip(response[i])
                }</td><td></td><td><i class="sb-icon-delete"></i></td></tr>`;
              }
            }
            chatbot_files_table
              .html(code[1])
              .sbLoading(false);
            chatbot_website_table
              .html(code[0])
              .sbLoading(false);
            chatbot_area
              .find("#sb-chatbot-delete-website")
              .setClass("sb-hide", !code[0]);
          }
        );
        SBF.ajax(
          {
          ...
          (response) => {
            ...
            let faq = response.map((item) => ({
              ...
              "open-ai-faq-function-calling-method":
                item[3],
              "open-ai-faq-function-calling-headers":
                item[4],
              "open-ai-faq-function-calling-properties":
                item[5] && item[5].length
                  ? item[5].map((item) => ({
                      name: item[0],
                      description: item[1],
                      allowed: item[2],
                  ...
                  : [["", "", ""]],
              "open-ai-faq-set-data":
                item[6] && item[6].length
                  ? item[6].map((item) => ({
                      id: item[0],
                      value: item[1],
                    }))
            ...
            if (faq.length) {
              chatbot_qea_repeater
                .find("> div > .sb-repeater")
                .html(
                  SBSettings.repeater.set(
                    faq,
                    chatbot_qea_repeater.find(
                      "> div > .sb-repeater > .repeater-item:last-child"
                    )
                  )
                );
              chatbot_qea_repeater
                ...
                .each(function () {
                  ...
                  if (
                    $(this).find("input").val() ||
                    no_value_keys.includes(selects.val())
                  ) {
                    $(this).sbActive(true);
                    if (
                      $(this).hasClass(
                        "sb-enlarger-function-calling"
                      )
                    ) {
                      $(this)
                        .closest(".repeater-item")
                        .find(".sb-qea-repeater-answer")
                        .addClass("sb-hide");
                    }
                  }
                  selects.each(function () {
                    if (
                      no_value_keys.includes($(this).val())
                    ) {
                      $(this)
                        .parent()
                        .next()
      ...
      send: function (
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            SBApps.unsupportedRichMessages(
              message,
              "Messenger"
            );
          }
    ...
    aecommerce: {
      conversationPanel: function () {
        let code = "";
        let aecommerce_id =
          activeUser().getExtra("aecommerce-id");
        if (!this.panel)
          this.panel = conversations_area.find(
            ".sb-panel-aecommerce"
          );
        if (aecommerce_id && !loading(this.panel)) {
          SBF.ajax(
            {
              function:
                "aecommerce-get-conversation-details",
            ...
            (response) => {
              ...
              code +=
                (response.cart.length
                  ? ""
                  : "<p>" +
                    sb_("The cart is currently empty.") +
                    "</p>") + "</div>";
    ...
    martfury: {
      conversationPanel: function () {
        let code = "";
        let martfury_id =
          activeUser().getExtra("martfury-id");
        if (!this.panel)
          this.panel = conversations_area.find(
            ".sb-panel-martfury"
          );
    ...
    ump: {
      conversationPanel: function () {
        ...
        if (!this.panel)
          this.panel =
            conversations_area.find(".sb-panel-ump");
        let code = "";
        let subscriptions;
        SBF.ajax(
          {
          ...
          (response) => {
            ...
            if (subscriptions.length) {
              ...
              code += `</div><span class="sb-title">${sb_(
                "Total spend"
              )} ${response.total} ${
                response.currency_symbol
              }</span>`;
    ...
    zendesk: {
      conversationPanel: function () {
        ...
        let zendesk_id =
          activeUser().getExtra("zendesk-id");
        let phone = activeUser().getExtra("phone");
        let email = activeUser().get("email");
        let panel = conversations_area.find(
          ".sb-panel-zendesk"
        );
        if (
          (zendesk_id || phone || email) &&
          !loading(panel)
        ) {
          SBF.ajax(
            {
            ...
            (response) => {
              ...
              panel
                .find(".sb-zendesk-date")
                .each(function () {
                  $(this).html(
                    SBF.beautifyTime($(this).html())
                  );
                });
              collapse(panel, 160);
            }
    ...
    woocommerce: {
      ...
      conversationPanel: function () {
        ...
        SBF.ajax(
          {
            function:
              "woocommerce-get-conversation-details",
          },
          (response) => {
      ...
      conversationPanelOrder: function (order_id) {
        let accordion = this.panel.find(
          `[data-id="${order_id}"] > div`
        );
        accordion.html("");
        SBF.ajax(
          {
          ...
          (response) => {
            let code = "";
            let collapse = this.panel.find(
              ".sb-collapse-btn:not(.sb-active)"
            );
            if (response) {
      ...
      conversationPanelUpdate: function (
        product_id,
        action = "added"
      ) {
        let busy = false;
    ...
    opencart: {
      conversationPanel: function () {
        ...
        let opencart_id =
          activeUser().getExtra("opencart_id");
        let store_url = activeUser().getExtra(
          "opencart_store_url"
        );
        if (!opencart_id) {
          return panel.html("");
    ...
    unsupportedRichMessages: function (
      message,
      app_name,
      unsupported = []
    ) {
        ...
        if (message.includes("[" + unsupported[i])) {
          infoBottom(
            "The {R} rich message is not supported by {R2}. The rich message was not sent to {R2}."
              .replace(
                /{R}/g,
                SBF.slugToString(unsupported[i])
              )
    ...
    itemsPanel: {
      ...
      code: function (items, app_name, label = true) {
        ...
        for (var i = 0; i < items.length; i++) {
          code += `<li data-id="${items[i].id
            .split("/")
            .pop()}"><div class="sb-image" style="background-image:url('${
            items[i].image
              ? app_name == "shopify"
                ? items[i].image.replace(
                    ".jpg",
                    "_small.jpg"
                  )
                : items[i].image
              : SB_URL + "/media/thumb.svg"
          }')"></div><div><span>${
            items[i].name ? items[i].name : items[i].title
          }</span><span>${
            items[i].price
              ? items[i].price
              : items[i].variants.edges[0].node.price
          } ${
            SB_ADMIN_SETTINGS.currency
          }</span></div></li>`;
        }
        return label
          ? code
            ? code
            : `<p class="sb-no-results">${sb_(
                "No products found"
              )}</p>`
          : code;
      },
      ...
      getAppInfo: function (app_name) {
        switch (app_name) {
          case "woocommerce":
            return {
              area: woocommerce_products_box,
              ul: woocommerce_products_box_ul,
              search: "woocommerce-search-products",
              filter: "woocommerce-get-products",
              populate: "woocommerce-products-popup",
              pagination: "woocommerce-get-products",
            };
          case "shopify":
            return {
              area: SBCloud.shopify_products_box,
              ul: SBCloud.shopify_products_box_ul,
              search: "shopify-get-products",
              filter: "shopify-get-products",
              populate: "shopify-get-products",
              pagination: "shopify-get-products",
            };
        }
      },
          ...
          if (search) {
            ...
            SBF.ajax(
              ...
              (response) => {
                ...
                this.getAppInfo(app_name).ul.html(
                  this.code(response, app_name)
                );
                $(icon).sbLoading(false);
              }
            );
      ...
      filter: function (item, app_name) {
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            app_info.ul
              .html(this.code(response, app_name))
              .sbLoading(false);
          }
        );
      },
      ...
      populate: function (app_name, onSuccess = false) {
        ...
        this.panel_language =
          activeUser() &&
          SB_ADMIN_SETTINGS.languages &&
          SB_ADMIN_SETTINGS.languages.includes(
            activeUser().language
          )
            ? activeUser().language
            : "";
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            select
              .find("ul")
              .html(
                `<li data-value="" class="sb-active">${sb_(
                  "All"
                )}</li>` + code
              );
            app_info.ul
              .html(this.code(response[0], app_name))
              .sbLoading(false);
            if (onSuccess !== false) {
              onSuccess();
            }
      ...
      pagination: function (area, app_name) {
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            app_info.ul
              .append(this.code(response, app_name, false))
              .sbLoading(false);
            if (!response.length) {
  ...
  var SBSettings = {
    ...
    save: function (btn = false) {
      ...
      switch (tab) {
        case "tab-automations":
          let active = automations_area_nav
            .find(".sb-active")
            .attr("data-id");
            ...
            .each((i, element) => {
              ...
              if (setting[0]) {
                if (typeof setting_id != ND) {
                  let originals = false;
                  if (
                    $(element).find("[data-language]")
                      .length
                  ) {
                    let language = $(element).find(
                      "[data-language].sb-active"
                    );
                    originals =
                      setting[0] in
                      this.translations.originals
                        ? this.translations.originals[
                            setting[0]
                          ]
                        : false;
    ...
    get: function (item) {
      ...
      switch (type) {
        ...
        case "select-images":
          return [
            id,
            item.find(".input > .sb-active").data("value"),
            type,
          ];
        case "repeater":
          return [
    ...
    set: function (id, setting) {
      ...
      switch (type) {
        ...
        case "select-checkbox":
          ...
          break;
        case "repeater":
          let content = this.repeater.set(
            value,
            settings_area.find(
              id + " .repeater-item:last-child"
            )
          );
          if (content) {
            settings_area
    ...
    repeater: {
      set: function (values, repeater_item_content) {
        ...
        if (values.length) {
          ...
          for (var i = 0; i < values.length; i++) {
            let item = $(
              $.parseHTML(
                `<div>${repeater_item_content}</div>`
              )
            );
            for (var key in values[i]) {
              SBSettings.input.set(
                item.find(`[data-id="${key}"]`),
                values[i][key]
              );
            }
      ...
      get: function (items) {
        ...
        $(items).each(function () {
          ...
          $(this)
            .find("[data-id]")
            .removeClass("sb-exclude");
          $(this)
            .find(".sb-repeater [data-id]")
            .addClass("sb-exclude");
          $(this)
            .find("[data-id]:not(.sb-exclude)")
            .each(function () {
      ...
      add: function (item) {
        ...
        item = $(
          $.parseHTML(
            `<div>${parent
              .find(
                "> .sb-repeater > .repeater-item:last-child"
              )
              .html()}</div>`
          )
        );
        this.clear(item);
        item
          .find(".repeater-item:not(:first-child)")
          .remove();
        ...
        parent
          .find("> .sb-repeater")
          .append(
            `<div class="repeater-item">${item.html()}</div>`
          );
      },
      ...
      delete: function (item) {
        ...
        if (
          cnt.parent().find(".sb-repeater-upload").length
        ) {
          SBF.ajax({
            function: "delete-file",
            path: parent.find("input").val(),
          });
        }
        if (cnt.find("> .repeater-item").length > 1) {
          ...
          parent
            .find('[data-id]:not([data-type="auto-id"]')
            .each((e, element) => {
      ...
      clear: function (item) {
        ...
        item
          .find('input:not([data-type="auto-id"]')
          .removeAttr("value checked");
    ...
    input: {
          ...
          if (input.hasClass("sb-repeater")) {
            input.html(
              SBSettings.repeater
                ...
                .replaceAll(
                  "sb-icon-close",
                  "sb-icon-close sb-sub-repeater-close"
                )
            );
          } else if (
            div ||
      ...
      get: function (input) {
        ...
        if (input.is(":checkbox")) {
          ...
          return SBSettings.repeater.get(
            input.find("> .repeater-item")
          );
        } else if (
          input.is("div") ||
          input.is("i") ||
      ...
      reset: function (input) {
        ...
        if (input.is("select")) {
          ...
          input
            .removeAttr("value style data-value")
            .val("");
        }
      },
    },
    ...
    visibility: function (index, visible) {
      let selectors = [
        ...
        [
          "#messenger-key, #messenger-path-btn",
          "#messenger-sync-btn",
        ],
        [
          "#open-ai-assistant-id",
          "#open-ai-prompt,#open-ai-model, #open-ai-tokens, #open-ai-temperature, #open-ai-presence-penalty, #open-ai-frequency-penalty, #open-ai-logit-bias, #open-ai-custom-model, #open-ai-source-links",
        ],
      ];
      settings_area
        .find(selectors[index][0])
        .sbActive(!visible);
      settings_area
        .find(selectors[index][1])
        .setClass("sb-hide", !visible);
    },
    ...
    automations: {
      ...
      conditions: function () {
        ...
        let user_extra_details =
          SBUsers.getExtraDetailsList(true);
        for (
          var i = 0;
          i < user_extra_details.length;
          i++
        ) {
          list[user_extra_details[i][0]] = [
            user_extra_details[i][1],
            [
              "Contains",
              "Does not contain",
              "Is set",
      ...
      show: function (id = false, language = false) {
        ...
        let items = language
          ? language in this.translations
            ...
            : []
          : this.items;
        let area = automations_area.find(
          " > .sb-tab > .sb-content"
        );
      ...
      add: function () {
        let id = SBF.random();
        let name = `${sb_("Item")} ${
          automations_area_nav.find(
            "li:not(.sb-no-results)"
          ).length + 1
        }`;
        this.updateActiveItem();
        ...
        automations_area
          ...
          .find("input, textarea")
          .val("");
        automations_area.sbLanguageSwitcher(
          [],
          "automations"
        );
      ...
      delete: function (element) {
        this.items[this.activeType()].splice(
      ...
      populate: function (type = false) {
        ...
        if (items.length) {
          ...
          code = `<li class="sb-no-results">${sb_(
            "No results found."
          )}</li>`;
        }
        ...
        switch (type) {
          case "emails":
            code = `<h2>${sb_(
              "Subject"
            )}</h2><div class="sb-setting sb-type-text"><div><input data-id="subject" type="text"></div></div>`;
            break;
          case "popups":
            code = `<h2>${sb_(
              "Title"
            )}</h2><div class="sb-setting sb-type-text"><div><input data-id="title" type="text"></div></div><h2>${sb_(
              "Profile image"
            )}</h2><div data-type="upload-image" class="sb-setting sb-type-upload-image"><div class="input"><div data-id="profile_image" class="image"><i class="sb-icon-close"></i></div></div></div><h2>${sb_(
              "Message fallback"
            )}</h2><div class="sb-setting sb-type-checkbox"><div><input data-id="fallback" type="checkbox"></div></div>`;
            break;
          case "design":
            code = `<h2>${sb_(
              "Header title"
            )}</h2><div class="sb-setting sb-type-text"><div><input data-id="title" type="text"></div></div>`;
            for (var i = 1; i < 4; i++) {
              code += `<h2>${sb_(
                (i == 1
                  ? "Primary"
                  ...
                  ? "Secondary"
                  : "Tertiary") + " color"
              )}</h2><div data-type="color" class="sb-setting sb-type-color"><div class="input"><input data-id="color_${i}" type="text"><i class="sb-close sb-icon-close"></i></div></div>`;
            }
            for (var i = 1; i < 4; i++) {
              code += `<h2>${sb_(
                i == 1
                  ...
                  ? "Header brand image"
                  : "Chat button icon"
              )}</h2><div data-type="upload-image" class="sb-setting sb-type-upload-image"><div class="input"><div data-id="${
                i == 1
                  ? "background"
                  : i == 2
                  ? "brand"
          ...
          case "more":
            code = `<h2>${sb_(
              "Department ID"
            )}</h2><div class="sb-setting sb-type-number"><div><input data-id="department" type="number"></div></div><h2>${sb_(
              "Agent ID"
            )}</h2><div class="sb-setting sb-type-number"><div><input data-id="agent" type="number"></div></div><h2>${sb_(
              "Tags"
            )}</h2><div class="sb-setting sb-type-text"><div><input data-id="tags" type="text"></div></div><h2>${sb_(
              "Article IDs"
            )}</h2><div class="sb-setting sb-type-number"><div><input data-id="articles" type="text"></div></div><h2>${sb_(
              "Articles category"
            )}</h2><div class="sb-setting sb-type-number"><div><input data-id="articles_category" type="text"></div></div>`;
            break;
        }
        automations_area
          .find(".sb-automation-extra")
          .html(code);
          ...
          for (var i = 0; i < items.length; i++) {
            if (items[i].id == id) {
              ...
              automations_area
                .find(".sb-automation-values")
                .find(
                  'input,textarea,[data-type="upload-image"] .image'
                )
                .each(function () {
              ...
              items[i].conditions =
                this.getConditions(conditions_area);
      ...
      setConditions: function (
        ...
        if (conditions) {
          for (var key in conditions) {
            ...
            let condition = conditions_area.find(
              " > div:last-child"
            );
            condition
              .find("select")
              .val(conditions[key][0]);
      ...
      addCondition: function (conditions_area) {
        conditions_area.append(
          `<div><div class="sb-setting sb-type-select sb-condition-1"><select>${this.getAvailableConditions()}</select></div></div>`
        );
      },
      ...
      updateCondition: function (element) {
        ...
        if ($(element).val()) {
          let condition =
            this.conditions()[$(element).val()];
          let code = "";
          if (condition[1].length) {
            code =
              '<div class="sb-setting sb-type-select sb-condition-2"><select>';
          ...
          parent.append(
            code +
              (condition.length > 2
                ? `<div class="sb-setting sb-type-text"><input placeholder="${sb_(
                    condition[2]
                  )}" type="text"></div>`
                : "")
          );
          parent
            ...
            .each(function () {
              let value = $(this).val();
              $(this).html(
                SBSettings.automations.getAvailableConditions(
                  [value]
                )
              );
              $(this).val(value);
      ...
      getAvailableConditions: function (
        ...
        for (var key in conditions) {
          if (
            ...
            (!existing_conditions.includes(key) ||
      ...
      addTranslation: function (
        ...
        if (this.getTranslations(id).includes(id)) {
          return console.warn(
            "Automation translation already in array."
          );
        }
        ...
        this.translations[language][type].push(
          this.itemArray(type, id)
        );
      },
      ...
      deleteTranslation: function (id = false, language) {
        ...
        if (language in this.translations) {
          ...
          for (var key in types) {
            ...
            for (var i = 0; i < items.length; i++) {
              if (items[i].id == id) {
                this.translations[language][key].splice(
                  i,
                  1
                );
                return true;
      ...
      activeType: function () {
        return automations_area_select
          .find("li.sb-active")
          .data("value");
      },
    ...
    translations: {
      ...
      add: function (language) {
        let setting = SBSettings.getSettingObject(
          language_switcher_target
        );
      ...
      delete: function (setting, language) {
        ...
        setting
          .find(
            `.sb-language-switcher [data-language="${language}"]`
          )
          .remove();
        ...
        for (var key in translations) {
          area
            .find(
              ' > [data-area="' +
                key +
                '"] .sb-setting:not(.sb-new-translation)'
            )
            .each(function () {
              translations[key][
        ...
        SBF.ajax(
          ...
          (translations) => {
            ...
            for (var i = 0; i < areas.length; i++) {
              let translations_area =
                translations[areas[i]];
              code += `<div${
                !i ? ' class="sb-active"' : ""
              } data-area="${areas[i]}">`;
  ...
  var SBArticles = {
    ...
    save: function (onSuccess = false) {
      ...
      article = {
        id: id,
        title: articles_area
          .find(".sb-article-title input")
          .val(),
        content: articles_area
          .find(".sb-article-content textarea")
          .val(),
        link: articles_area
          .find(".sb-article-link input")
          .val(),
        parent_category:
          articles_category_parent_select.val(),
          ...
          if (is_number) {
            ...
            if (article.language) {
              ...
              for (
                ...
                if (
                  translations[i][0] == article.language
                ) {
                  translations[i][1] = article.id;
                  SBArticles.translations.list[
                    article.parent_id
                  ] = translations;
                  break;
                }
    ...
    show: function (article_id) {
      ...
      this.get((response) => {
        ...
        if (
          editor_js ||
          articles_area.find("#editorjs").length
        ) {
          editorJSLoad(
            response.editor_js
    ...
    populate: function (items, is_category = false) {
      ...
      articles_area
        .find(
          is_category ? ".ul-categories" : ".ul-articles"
        )
        .html(code);
      articles_area
        .find(
      ...
      if (this.page_url) {
        ...
        button.attr(
          ...
          article_id
            ? this.page_url +
                (this.is_url_rewrite
                  ? (this.page_url.charAt(
                      this.page_url.length - 1
                    ) == "/"
                      ? ""
                      : "/") +
                    (this.cloud_chat_id
    ...
    categories: {
      ...
      show: function (category_id, language = false) {
        ...
        if (index !== false) {
          let category = language
            ...
            : this.list[index];
          let image = articles_content_categories.find(
            "#category-image"
          );
          this.updateActive();
          articles_content_categories
          ...
          articles_content_categories
            ...
            .prop(
              "checked",
              category.parent ? true : false
            );
          articles_content_categories.sbLanguageSwitcher(
            $.map(
              this.list[index].languages,
              function (e, index) {
                return index;
              }
            ),
      ...
      add: function () {
        ...
        articles_area
          ...
          .eq(
            articles_area.find(".ul-categories li").length -
              1
          )
          .click();
      ...
      update: function () {
        let selected_category =
          articles_category_select.val();
        let selected_category_parent =
          articles_category_parent_select.val();
        ...
        if (this.list.length) {
          articles_category_parent_select.val(
            ids.includes(selected_category_parent)
              ? selected_category_parent
              : articles_category_parent_select[0]
                  .selectedIndex > -1
              ? this.list[
                  articles_category_parent_select[0]
                    .selectedIndex
          ...
          articles_category_select.val(
            ids.includes(selected_category)
              ? selected_category
              : articles_category_select[0].selectedIndex >
                -1
              ? this.list[
                  articles_category_select[0].selectedIndex
                ].id
      ...
      updateActive: function () {
        ...
        if (id) {
          ...
          let language = articles_content_categories
            ...
            .attr("data-language");
          let category = {
            title: articles_content_categories
              .find("#category-title")
              .val(),
      ...
      translations: {
        ...
        delete: function (
          ...
          if (!category_id) {
            category_id =
              SBArticles.categories.activeID(active_id);
          }
    ...
    translations: {
      ...
      get: function (article_id) {
        return article_id in this.list
          ? this.list[article_id]
          : [];
      },
    },
  };
      ...
      this.getData(
        ...
        (response) => {
          if (response == false) {
            ...
            reports_area
              .find(".sb-reports-title")
              .html(response.title);
            reports_area
              .find(".sb-reports-text")
              .html(response.description);
            reports_area.find(".sb-collapse-btn").remove();
            if (!responsive) {
              collapse(
                reports_area.find(".sb-collapse"),
                reports_area.find("canvas").outerHeight() -
                  135
              );
            }
          }
  ...
  var SBUsers = {
    ...
    get: function (
      onSusccess,
      is_online_users = false,
      is_users_pagination = false
    ) {
      let filters = [];
      ...
      SBF.ajax(
        {
          function: is_online_users
            ? "get-online-users"
            : "get-users",
          sorting: is_online_users
            ? this.sorting[0]
            : this.sorting,
          pagination: is_users_pagination
            ? users_pagination
            : false,
    ...
    populate: function (response) {
      ...
      if (count) {
        for (var i = 0; i < count; i++) {
          code += this.getRow(
            new SBUser(response[i], response[i].extra)
          );
        }
      } else {
        code = `<p class="sb-no-results">${sb_(
          "No users found."
        )}</p>`;
      }
    ...
    update: function () {
      if (!this.busy) {
        ...
        let populate =
          checks.includes(this.user_types[0]) &&
          !this.search_query;
        let filter = users_table_menu
          .find(".sb-active")
          .data("type");
        if (filter == "online") {
          this.filter(filter);
              ...
              if (count > 0) {
                ...
                if (populate) {
                  ...
                  if (checks.includes(filter)) {
                    ...
                    users_table
                      .find(selector.slice(0, -1))
                      .remove();
                  }
                }
                this.datetime_last_user =
                  response[0].creation_time;
              }
            }
          );
        }
    ...
    updateRow: function (user) {
      let row = users_table.find(
        `[data-user-id="${user.id}"]`
      );
      if (row.length) {
        let menu_active = users_table_menu
          .find(".sb-active")
          .data("type");
        if (
          ...
          let counter = admin.find(
            `[data-type="${
              user.type == "admin" ? "agent" : user.type
            }"] span`
          );
    ...
    updateMenuItem: function (
      ...
      let item = users_table_menu.find(
        `[data-type="${type}"] span`
      );
      ...
      if (Array.isArray(user_ids)) {
        ...
        SBF.ajax(
          ...
          () => {
            ...
            if (
              users_table.find("[data-user-id]").length == 0
            ) {
              this.filter(
                users_table_menu
                  .find(".sb-active")
        ...
        users[user_ids].delete(() => {
          let conversation =
            conversations_admin_list_ul.find(
    ...
    csv: function () {
      SBF.ajax(
        {
          function: "csv-users",
          users_id: SBUsers.getSelected(),
        },
        (response) => {
          dialogDeleteFile(
            response,
            "sb-export-users-close",
            "Users exported"
          );
    ...
    setActiveAgentStatus: function (online = true) {
      ...
      if (SBPusher.active) {
        if (online) {
          SBPusher.presence();
          if (
            SB_ADMIN_SETTINGS.routing == "queue" ||
            SB_ADMIN_SETTINGS.routing == "routing"
          ) {
            SBF.ajax(
              {
                function:
                  "assign-conversations-active-agent",
                is_queue:
                  SB_ADMIN_SETTINGS.routing == "queue",
              },
              () => {
    ...
    setActiveUserStatus: function (online = true) {
      ...
      if (online)
        labels.prepend(
          `<span class="sb-status-online">${sb_(
            "Online"
          )}</span>`
        );
    ...
    onlineUserNotification: function (member) {
      let notification =
        SB_ADMIN_SETTINGS.online_users_notification;
      if (notification) {
        let message =
          member.info.first_name +
          " " +
          member.info.last_name;
        let icon = this.userProfileImage(
          member.info.profile_image
        );
        if (
    ...
    getExtraDetailsList: function (only_custom = false) {
      return profile_edit_box
        .find(
          ".sb-additional-details .sb-edit-box > " +
            (only_custom
              ? ".sb-custom-detail"
              : ".sb-input")
        )
  ...
  var SBConversations = {
    ...
    openConversation: function (
      ...
      if (user_id === false && conversation_id) {
        ...
        let new_user =
          SBF.null(users[user_id]) ||
          !users[user_id].details.email;
        let conversation = conversations_area.find(
          `[data-conversation-id="${conversation_id}"]`
        );
        let conversation_list =
          conversations_admin_list_ul.find("li");
        ...
        if (SBPusher.active) {
          ...
          SBPusher.event(
            ...
            (response) => {
              if (
                response.previous_conversation_id ==
                conversation_id
              ) {
                conversations_area
                  .find(".sb-conversation-busy")
                  .remove();
              }
            },
            "agents"
          ...
          activeUser().getFullConversation(
            ...
            (response) => {
              ...
              if (SBAdmin.must_translate) {
                ...
                for (
                  ...
                  if (message.message) {
                    if (
                      ...
                      message_user_types.push(
                        message.get("user_type")
                      );
                    }
                  }
                }
                  ...
                  SBApps.dialogflow.translate(
                    ...
                    (response_translate) => {
                      if (response_translate) {
                        for (
                          var i = 0;
                          i < response_translate.length;
                          i++
                        ) {
                          let message =
                            SBChat.conversation.getMessage(
                              message_ids[i]
                            );
                          message.payload(
                            "translation",
                ...
                for (
                  var i = 0;
                  i < messages_translated.length;
                  i++
                ) {
                  this.openConversation_2(
                    messages_translated[i].id,
                    messages_translated[i].get("user_type")
                  );
                }
              }
              ...
              if (select_departments.length) {
                let department = response.get("department")
                  ? this.getDepartments(
                      response.get("department")
                    )
                  : false;
                let color = department
                  ? department["department-color"]
                  : "";
                if (!SB_ADMIN_SETTINGS.departments_show) {
                  conversation_list.attr("data-color", "");
                }
                getListConversation(conversation_id).attr(
                  "data-color",
                  color
                );
                select_departments
              ...
              // Agent assignment
              let select_agents = conversations_area.find(
                "#conversation-agent"
              );
              ...
              if (
                ...
                select
                  .find(
                    `[data-value="${conversation_status_code}"]`
                  )
                  .click();
              ...
              if (
                ...
                conversations_admin_list_ul.prepend(
                  SBConversations.getListCode(response)
                );
              }
              conversations_admin_list_ul
                .find("li")
              ...
              this.notificationsCounterReset(
                conversation_id,
                conversation
              );
              ...
              for (
                ...
                let payload =
                  response.messages[i].get("payload");
                if (payload.rating) {
                  conversations_area
              ...
              if (response.get("source") == "wa") {
                ...
                if (datetime) {
                  if (
                    ...
                    infoBottom(
                      sb_(
                        "You can't send a WhatsApp message more than 24 hours after the user's last message—use a template instead."
                      ) +
                        '<i id="sb-whatsapp-alert-btn" class="sb-icon-social-wa"</i>',
                      "info"
                    );
                  }
                }
              ...
              admin.on(
                ...
                function () {
                  SBConversations.showDirectMessageBox(
                    "whatsapp",
                    [activeUser().id]
                  );
                }
              ...
              activeUser().getConversations(function (
                response
              ) {
                conversations_area
                  ...
                  .html(
                    response.length == 1
                      ? ""
                      : activeUser().getConversationsCode(
                          response
                        )
                  )
                  .prev()
              ...
              // Search
              if (this.is_search) {
                let search = conversations_admin_list
    ...
    openConversation_1: function (
      ...
      if (message && message.payload("sb-human-takeover")) {
        message = response.getLastUserMessage(
          message.get("index")
        );
      }
      if (message) {
    ...
    openConversation_2: function (message_id, user_type) {
      ...
      if (message) {
        ...
        conversations_area_list
          ...
          .prepend(
            `<li data-value="${
              is_translation ? "translation" : "original"
            }">${sb_(
              is_translation
                ? "View translation"
            ...
            if (response.length) {
              ...
              for (var i = 0; i < response.length; i++) {
                if (!id_check.includes(response[i].id)) {
                  ...
                  let conversation_li =
                    getListConversation(conversation_id);
                  let is_existing_conversation =
                    conversation_li.length;
                  ...
                  if (
                    ...
                    (is_user ||
                      message.payload(
                        "human-takeover-message-confirmation"
                      ))
                  ) {
                  ...
                  let conversation_code = this.getListCode(
                    conversation,
                    null
                  );
                  ...
                  if (active_conversation) {
                    ...
                    conversations[conversation_li.index()] =
                      conversation;
                    getListConversation(
                      conversation_id
                    ).remove();
                  ...
                  if (!(user_id in users)) {
                    users[user_id] = new SBUser({
                      id: user_id,
                      first_name:
                        conversation.get("first_name"),
                      last_name:
                        conversation.get("last_name"),
                      profile_image:
                        conversation.get("profile_image"),
                      user_type: user_type,
                    });
                  }
              ...
              for (
                ...
                for (
                  ...
                  if (
                    conversations[j].id ==
                    SBChat.notifications[i][0]
                  ) {
                    is_found =
                      conversations[j].status_code == 2;
                    break;
                  }
                }
                if (!is_found) {
    ...
    messageMenu: function (
      ...
      let code =
        ...
        (SB_ADMIN_SETTINGS.supervisor &&
          ...
          : "") +
        (replay
          ? `<li data-value="reply">${sb_("Reply")}</li>`
          : "");
    ...
    updateUserDetails() {
      ...
      conversations_area
        .find(
          `[data-user-id="${activeUser().id}"] .sb-name`
        )
    ...
    setReadIcon(status_code) {
      ...
      conversations_area
        ...
        .attr(
          "class",
          unread ? "sb-icon-check-circle" : "sb-icon-circle"
        );
    },
    ...
    setStatus(
      ...
      if (conversation_id) {
        conversations[
          getListConversation(conversation_id).index()
    ...
    getListCode: function (conversation, status) {
      ...
      let label_new = "";
      let tags = SB_ADMIN_SETTINGS.tags_show
      ...
      if (
        ...
        notification_counter = SBF.storage(
          "notifications-counter"
        );
        if (
      ...
      if (!time) {
        time = conversation.getLastMessage()
          ? conversation
              .getLastMessage()
              .get("creation_time")
          : conversation.get("creation_time");
      }
      return `<li${
        is_active ? ' class="sb-active"' : ""
      } data-user-id="${conversation.get(
        "user_id"
        ...
        !SBF.null(conversation.get("source"))
          ? ` data-conversation-source="${conversation.get(
              "source"
            )}"`
          : ""
      }>${
    ...
    typing: function (typing) {
      if (typing) {
        ...
        conversations_area
          .find(
            ".sb-conversation .sb-top .sb-status-typing"
          )
          .remove();
    ...
    notificationsCounterReset: function (
      ...
      let notification_counter = SBF.storage(
        "notifications-counter"
      );
      if (
        notification_counter &&
    ...
    ucurl(url) {
      ...
      conversations_area
        .find(
          '.sb-profile-list [data-id="current_url"] label'
        )
        .attr("data-value", url)
        .html(url);
      if (extra) {
    ...
    setActiveDepartment: function (department_id) {
      if (
        SBChat.conversation &&
        SBChat.conversation.get("department") ==
          department_id
      ) {
        return;
      }
      let department = department_id
        ? this.getDepartments(department_id)
        : false;
      let departmnet_color = department
        ? department["department-color"]
        : "";
      let conversation_li = getListConversation(
        SBChat.conversation.id
      );
    ...
    getDepartments: function (department_id = false) {
      if (department_id) {
        for (
          var i = 0;
          i < SB_ADMIN_SETTINGS.departments.length;
          i++
        ) {
          if (
            SB_ADMIN_SETTINGS.departments[i][
              "department-id"
            ] == department_id
          ) {
    ...
    setActiveAgent: function (agent_id) {
      let select = conversations_area.find(
        "#conversation-agent"
      );
    ...
    clickFirst: function (conversation_li = false) {
      if (!conversation_li) {
        conversation_li = conversations_admin_list_ul.find(
          "li:first-child"
        );
      }
      if (conversation_li.length) {
        ...
        if (
          !conversations_admin_list_ul.find("li").length
        ) {
    ...
    savedReplies: function (textarea, value) {
      let last_char = value.charAt(
        textarea.selectionStart - 1
      );
      let pre_text = value.substr(
        0,
        value.lastIndexOf("#")
      );
      if (last_char == "#") {
        if (
          ...
          $(textarea).val(
            pre_text.substr(0, pre_text.length - 1)
          );
          return conversation_area
            .find(".sb-btn-saved-replies")
            .click();
        }
      ...
      if (SBChat.editor_listening && last_char == " ") {
        ...
        for (
          var i = 0;
          i < saved_replies_list.length;
          i++
        ) {
          if (
            saved_replies_list[i]["reply-name"] == keyword
          ) {
            $(textarea).val(
              pre_text + saved_replies_list[i]["reply-text"]
            );
            return;
          }
        }
    ...
    attachments: function () {
      if (attachments_panel.length) {
        let attachments =
          SBChat.conversation.getAttachments();
        let code = "";
        let code_filters = "";
        let file_types = [];
        for (var i = attachments.length - 1; i > -1; i--) {
          let file_type = SBF.getFileType(
            attachments[i][1]
          );
        ...
        if (
          ...
          code_filters = `<div id="sb-attachments-filter" class="sb-select"><p>${sb_(
            "All"
          )}</p><ul><li data-value="">${sb_("All")}</li>`;
          for (var i = 0; i < file_types.length; i++) {
            code_filters += `<li data-value="${
              file_types[i]
            }">${sb_(
              SBF.slugToString(file_types[i]) + "s"
            )}</li>`;
          }
          code_filters += `</ul></div>`;
        }
        $(attachments_panel).html(
          code
            ? `<h3${
                code_filters ? ' class="sb-flex"' : ""
              }>${sb_(
                "Attachments"
              )}${code_filters}</h3><div class="sb-list-items sb-list-links sb-list-icon">${code}</div>`
            : ""
        );
    ...
    filters: function () {
      ...
      for (
        var i = 0;
        i < conversations_filters.length;
        i++
      ) {
        values.push(
          conversations_filters
    ...
    tags: {
      ...
      get: function (tag_name) {
        for (
          var i = 0;
          i < SB_ADMIN_SETTINGS.tags.length;
          i++
        ) {
          if (
            tag_name ==
            SB_ADMIN_SETTINGS.tags[i]["tag-name"]
          ) {
      ...
      code: function (index_or_tag, active) {
        ...
        if (tag) {
          ...
          return `<span data-value="${name}" data-color="${
            tag["tag-color"]
          }"${active ? ' class="sb-active"' : ""}>${sb_(
            name
          )}</span>`;
        }
        return "";
  ...
  var SBProfile = {
    ...
    show: function (user_id) {
      ...
      activeUser().update(() => {
        this.populate(
          activeUser(),
          profile_box.find(".sb-profile-list")
        );
        ...
        activeUser().getConversations((response) => {
          ...
          profile_box
            .find(".sb-top-bar [data-value]")
            .sbActive(false);
          if (!SBF.null(activeUser().get("email"))) {
            profile_box
              .find('.sb-top-bar [data-value="email"]')
              .sbActive(true);
          }
          if (activeUser().getExtra("phone")) {
            if (SB_ADMIN_SETTINGS.sms) {
              profile_box
                .find('.sb-top-bar [data-value="sms"]')
                .sbActive(true);
            }
            profile_box
          ...
          profile_box
            .attr("data-user-id", activeUser().id)
            .sbShowLightbox();
    ...
    showEdit: function (user) {
      if (user instanceof SBUser) {
        let password = profile_edit_box.find(
          "#password input"
        );
        let current_user_type = user.type;
        let select = profile_edit_box.find(
          "#user_type select"
        );
        profile_edit_box
        ...
        profile_edit_box
          ...
          .html(
            `<i class="sb-icon-check"></i>${sb_(
              "Save changes"
            )}`
          );
        profile_edit_box.find(".sb-profile").setProfile();
        profile_edit_box
          .find(".sb-unlisted-detail")
          .remove();
        profile_edit_box
          .find("input,select,textara")
        ...
        for (var id in user.extra) {
          if (!default_ids.includes(id)) {
            code += `<div id="${id}" data-type="text" class="sb-input sb-unlisted-detail"><span>${sb_(
              user.extra[id].name
    ...
    populate: function (user, profile_area) {
      ...
      let code = "";
      if (
        profile_area.hasClass(
          "sb-profile-list-conversation"
        ) &&
        SBChat.conversation
      ) {
    ...
    profileRow: function (key, value, name = key) {
      ...
      switch (key) {
        ...
        case "browser_language":
          ...
          if (
            value == "ar" &&
            (key == "language" || key == "browser_language")
          ) {
            flag = "arc";
          }
    ...
    populateEdit: function (user, profile_edit_area) {
      profile_edit_area
        .find(".sb-details .sb-input")
        .each((i, element) => {
          this.set(
            element,
            user.details[$(element).attr("id")]
          );
        });
      profile_edit_area
    ...
    errors: function (profile_edit_area) {
      return SBForm.errors(
        profile_edit_area.find(".sb-details")
      );
    },
    ...
    boxClasses: function (box, user_type = false) {
      $(box)
        ...
        .addClass(
          `${
            user_type != false ? `sb-type-${user_type}` : ""
          } sb-agent-${SB_ACTIVE_AGENT.user_type}`
        );
    },
  ...
  var SBAdmin = {
    ...
    collapse(target, max_height) {
      ...
      if (
        ...
        target
          .sbActive(true)
          .attr("data-height", max_height);
        target.append(
          `<a class="sb-btn-text sb-collapse-btn">${sb_(
            "View more"
          )}</a>`
        );
        content.css({
  ...
  $(document).ready(function () {
    ...
    conversations_area = admin.find(
      ".sb-area-conversations"
    );
    conversation_area = conversations_area.find(
      ".sb-conversation"
    );
    conversations_area_list = conversations_area.find(
      ".sb-conversation .sb-list"
    );
    conversations_admin_list = conversations_area.find(
      ".sb-admin-list"
    );
    conversations_admin_list_ul =
      conversations_admin_list.find(".sb-scroll-area ul");
    conversations_filters =
      conversations_admin_list.find(".sb-select");
    conversations_user_details = conversations_area.find(
      ".sb-user-details"
    );
    ...
    automations_area = settings_area.find(
      ".sb-automations-area"
    );
    conditions_area = automations_area.find(
      ".sb-conditions"
    );
    automations_area_select =
      automations_area.find(" > .sb-select");
    automations_area_nav = automations_area.find(
      " > .sb-tab > .sb-nav > ul"
    );
    reports_area = admin.find(".sb-area-reports");
    articles_area = admin.find(".sb-area-articles");
    articles_content = articles_area.find(
      ".sb-content-articles"
    );
    articles_content_categories = articles_area.find(
      ".sb-content-categories"
    );
    articles_category_parent_select = admin.find(
      "#article-parent-categories"
    );
    articles_category_select = admin.find(
      "#article-categories"
    );
    ...
    notes_panel = conversations_area.find(
      ".sb-panel-notes"
    );
    tags_panel = conversations_area.find(".sb-panel-tags");
    attachments_panel = conversations_area.find(
      ".sb-panel-attachments"
    );
    direct_message_box = admin.find(
      ".sb-direct-message-box"
    );
    wp_admin =
      SBApps.is("wordpress") && $(".wp-admin").length;
    dialogflow_intent_box = admin.find(
      ".sb-dialogflow-intent-box"
    );
    suggestions_area = conversations_area.find(
      ".sb-editor > .sb-suggestions"
    );
    open_ai_button = conversations_area.find(
      ".sb-btn-open-ai"
    );
    select_departments = conversations_area.find(
      "#conversation-department"
    );
    upload_input = admin.find(
      ".sb-upload-form-admin .sb-upload-files"
    );
    chatbot_area = admin.find(".sb-area-chatbot");
    chatbot_files_table = chatbot_area.find(
      "#sb-table-chatbot-files"
    );
    chatbot_website_table = chatbot_area.find(
      "#sb-table-chatbot-website"
    );
    chatbot_qea_repeater = chatbot_area.find(
      "#sb-chatbot-qea"
    );
    chatbot_playground_editor = chatbot_area.find(
      ".sb-playground-editor"
    );
    chatbot_playground_area = chatbot_area.find(
      ".sb-playground .sb-scroll-area"
    );
    flows_area = chatbot_area.find(
    ...
    window.onpopstate = function () {
      ...
      if (SBF.getURL("user")) {
        ...
        header
          .find(".sb-admin-nav #sb-" + SBF.getURL("area"))
          .click();
      } else if (SBF.getURL("conversation")) {
        if (!conversations_area.sbActive()) {
          header
            .find(".sb-admin-nav #sb-conversations")
            .click();
        }
                ...
                if (response != false) {
                  ...
                  if (response === true) {
                    ...
                    switch (response) {
                      ...
                      case "missing-url":
                        message =
                          "Support Board cannot get the plugin URL.";
                        break;
                      default:
                        message = response;
                    }
    ...
    $(window).keydown(function (e) {
      ...
      active_keydown = code;
      if (
        [13, 27, 32, 37, 38, 39, 40, 46, 90].includes(code)
      ) {
        if (admin.find(".sb-dialog-box").sbActive()) {
          ...
          let editor = conversations_area.find(
            ".sb-editor textarea"
          );
          ...
          if (code == 46) {
            ...
            let target = conversations_area.find(
              " > div > .sb-conversation"
            );
            target
              .find(
          ...
          let target = users_table.find(
            `[data-user-id="${activeUser().id}"]`
          );
          target =
            code == 39 ? target.next() : target.prev();
          if (target.length) {
            admin.sbHideLightbox();
          ...
          code == 13 &&
          chatbot_area
            .find(".sb-playground-editor textarea")
            .is(":focus")
        ) {
          chatbot_area
            .find(
              '.sb-playground-editor [data-value="send"]'
            )
            .click();
          valid = true;
        }
      ...
      SBF.ajax(
        ...
        (response) => {
          ...
          for (var key in response) {
            if (SBApps.is(key)) {
              let updated =
                SB_VERSIONS[key] == response[key];
              if (!updated) {
                updates = true;
              }
              code += `<div class="sb-input"><span>${
                names[key]
              }</span><div${
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            if (
              SBF.errorValidation(
                response,
                "envato-purchase-code-not-found"
              )
            ) {
              error = messages[0];
              ...
              for (var key in response) {
                if (response[key] != "success") {
                  ...
                  if (
                    response[key] == "license-key-not-found"
                  ) {
                    error = sb_(
    ...
    $(admin).on(
      ...
      function () {
        ...
        let is_active =
          SBApps.is(app_name) &&
          (!is_cloud ||
            SB_CLOUD_ACTIVE_APPS.includes(app_name));
        let ga =
          "?utm_source=plugin&utm_medium=admin_area&utm_campaign=plugin";
        if (!is_cloud) {
          SBF.ajax(
            {
        ...
        box
          ...
          .attr(
            "href",
            "https://board.support/shop/" + app_name + ga
          );
        box
    ...
    $(admin).on(
      "click",
      ".sb-app-box .sb-btn-app-setting",
      function () {
        settings_area
          .find(
            "#tab-" +
              $(this).closest("[data-app]").attr("data-app")
          )
          .click();
        admin.sbHideLightbox();
      }
    ...
    if (
      ...
      (SB_ADMIN_SETTINGS.desktop_notifications == "all" ||
        SB_ADMIN_SETTINGS.desktop_notifications ==
          "agents") &&
      !SB_ADMIN_SETTINGS.push_notifications
    ) {
    ...
    if (
      ["all", "agents"].includes(
        SB_ADMIN_SETTINGS.flash_notifications
      )
    ) {
            ...
            menu
              ...
              .scrollTop(
                menu.next()[0].offsetTop -
                  (admin.hasClass("sb-header-hidden")
                    ? 70
                    : 130)
              );
          }
    ...
    $(header).on("click", " .sb-admin-nav a", function () {
      ...
      admin.find(" > main > div").sbActive(false);
      admin
        .find(".sb-area-" + active_admin_area)
        .sbActive(true);
      $(this).sbActive(true);
      SBF.deactivateAll();
      switch (active_admin_area) {
          ...
          if (!SBSettings.init) {
            ...
            SBF.ajax(
              ...
              (response) => {
                ...
                settings_area
                  .find(
                    "[data-setting] .sb-language-switcher-cnt"
                  )
                  .each(function () {
        ...
        case "reports":
          if (reports_area.sbLoading()) {
            $.getScript(
              SB_URL + "/vendor/moment.min.js",
              () => {
                $.getScript(
                  SB_URL + "/vendor/daterangepicker.min.js",
                  () => {
                    $.getScript(
                      SB_URL + "/vendor/chart.min.js",
                      () => {
                        SBReports.initDatePicker();
                        SBReports.initReport(
                          "conversations"
                        );
      ...
      if (
        ...
        ((active_admin_area == "conversations" &&
          !SBF.getURL("conversation")) ||
          (active_admin_area == "users" &&
            !SBF.getURL("user")) ||
          (active_admin_area == "settings" &&
            !SBF.getURL("setting")) ||
          (active_admin_area == "reports" &&
            !SBF.getURL("report")) ||
          (active_admin_area == "articles" &&
            !SBF.getURL("article")) ||
          (active_admin_area == "chatbot" &&
            !SBF.getURL("chatbot")))
      ) {
    ...
    $(conversations_admin_list_ul).on(
      "click",
      "li",
      function () {
      ...
      function () {
          ...
          conversations_admin_list_ul
            ...
            .map(function () {
              return {
                ...
                status_code: $(this).attr(
                  "data-conversation-status"
                ),
              };
            })
            .toArray();
        let selected_conversations_length =
          selected_conversations.length;
        let multi_selection =
          selected_conversations_length > 1;
        ...
        if (status_code) {
          infoPanel(message, "alert", function () {
            let active_conversations_filter =
              conversations_filters
                .eq(0)
                .find("p")
                .attr("data-value");
            ...
            for (
              var i = 0;
              i < selected_conversations_length;
              i++
            ) {
    ...
    $(admin).on("click", ".sb-btn-open-ai", function () {
      ...
      let is_editor = $(this).hasClass(
        "sb-btn-open-ai-editor"
      );
    ...
    $(conversations_admin_list)
      ...
      .on("scroll", function () {
        if (
          ...
          let parent = conversations_area.find(
            ".sb-admin-list"
          );
          let filters = SBConversations.filters();
          is_busy = true;
          parent.append(
            '<div class="sb-loading-global sb-loading"></div>'
          );
          SBF.ajax(
            {
              function: "get-conversations",
        ...
        for (var i = 0; i < messages.length; i++) {
          ...
          if (SBAdmin.must_translate) {
            ...
            if (message_original) {
              ...
              conversation_area
                ...
                .prepend(
                  `<li data-value="translation">${sb_(
                    "View translation"
                  )}</li>`
                );
              if (SB_ADMIN_SETTINGS.smart_reply) {
                SBApps.dialogflow.smartReply(
                  SBF.escape(message_original)
                );
              }
              ...
              SBApps.dialogflow.translate(
                ...
                (response_2) => {
                  if (response_2) {
                    ...
                    message_html.replaceWith(
                      message.getCode()
                    );
                    conversation_area
                      .find(
                        `[data-id="${message.id}"] .sb-menu`
                      )
                      .prepend(
          ...
          if (payload) {
            if (payload.department) {
              SBConversations.setActiveDepartment(
                payload.department
              );
            }
            if (payload.agent) {
    ...
    $(document).on("SBSMSSent", function () {
      infoBottom(
        "The user has been notified by text message."
      );
    });
    ...
    $(document).on(
      ...
      function (e, response) {
        infoBottom(
          `The user ${
            response.includes("cron")
              ? "will be"
              : "has been"
          } notified by email${
            response.includes("sms")
              ? " and text message"
              : ""
          }.`
        );
                ...
                if (!responsive) {
                  if (SBChat.conversation) {
                    ...
                    if (conversation.length) {
                      ...
                      filters[0] ==
                      SBChat.conversation.status_code
                    ) {
                      conversations_admin_list_ul.prepend(
                        SBConversations.getListCode(
                          SBChat.conversation
    ...
    $(admin).on(
      "click",
      ".sb-profile-list-conversation li",
      function () {
        ...
        switch ($(this).attr("data-id")) {
          ...
          case "conversation-source":
            ...
            if (
              ...
              window.open(
                "https://wa.me/" +
                  SBApps.whatsapp.activeUserPhone()
              );
            ...
            SBF.ajax(
              ...
              (response) => {
                ...
                if (response && response.item) {
                  ...
                  for (var key in response) {
                    if (
                      isString(response[key]) ||
                      !isNaN(response[key])
                    ) {
                      code += `<b>${SBF.slugToString(
                        key
                      )}</b> ${response[key]} <br>`;
                    }
                  }
          ...
          case "cc":
            if (
              SBChat.conversation &&
              SBChat.conversation.get("source") == "em"
            ) {
              let cc = SBChat.conversation
                .get("extra")
                .split(",");
    ...
    $(dialogflow_intent_box).on(
      ...
      function () {
        dialogflow_intent_box
          ...
          .last()
          .after(
            '<div class="sb-setting sb-type-text"><input type="text"></div>'
          );
      }
    );
      ...
      function () {
        ...
        for (var i = 0; i < messages_length; i++) {
          if (
            ...
            SBApps.openAI.generateQuestions(
              messages[i].message
            );
            break;
          }
    ...
    $(dialogflow_intent_box).on(
      ...
      "#sb-intent-preview",
      function () {
        SBApps.dialogflow.previewIntentDialogflow(
          dialogflow_intent_box
            .find("#sb-intents-select")
            .val()
        );
    ...
    $(dialogflow_intent_box).on(
      ...
      function () {
        SBApps.dialogflow.previewIntent(
          dialogflow_intent_box.find("#sb-qea-select").val()
        );
      }
    );
    ...
    $(dialogflow_intent_box).on(
      ...
      function () {
        ...
        dialogflow_intent_box
          ...
          .val(
            intent
              ? SBApps.dialogflow.getIntent(intent)
    ...
    $(dialogflow_intent_box).on(
      ...
      function () {
        ...
        dialogflow_intent_box
          .find(".sb-bot-response")
          .setClass("sb-disabled", qea)
          .find("textarea")
          .val(
            qea
              ? SBApps.dialogflow.qea[qea][1]
              : SBApps.dialogflow.original_response
          );
        dialogflow_intent_box
          .find("#sb-train-chatbots")
    ...
    $(dialogflow_intent_box).on(
      ...
      function () {
        ...
        timeout = setTimeout(() => {
          SBApps.dialogflow.original_response =
            dialogflow_intent_box.find("textarea").val();
        }, 500);
      }
    );
    ...
    notes_panel.on(
      ...
      function (e) {
        ...
        SBAdmin.genericPanel(
          ...
          `<div class="sb-setting sb-type-textarea"><textarea${
            note
              ? ' data-id="' + note.attr("data-id") + '"'
              : ""
          } placeholder="${sb_(
        ...
        if (
          SBApps.is("dialogflow") &&
          SB_ADMIN_SETTINGS.note_data_scrape
        ) {
          let options = "";
          ...
          admin
            ...
            .parent()
            .prepend(
              `<div id="note-ai-scraping" class="sb-setting sb-type-select"><select><option value="">${sb_(
                "Data scraping"
    ...
    tags_panel.on("click", "> i", function (e) {
      let code = SBConversations.tags.getAll(
        ...
        (response) => {
          ...
          if (response === true) {
            ...
            if (
              ...
              if (
                tag_filter &&
                !tags.includes(tag_filter)
              ) {
                conversation_li.remove();
                SBConversations.clickFirst();
              } else if (SB_ADMIN_SETTINGS.tags_show) {
                let tags_area =
                  conversation_li.find(".sb-tags-area");
                let code =
                  SBConversations.tags.codeLeft(tags);
                if (tags_area.length) {
                  tags_area.replaceWith(code);
                } else {
                  $(code).insertAfter(
                    conversation_li.find(".sb-name")
                  );
                }
              }
            }
    ...
    $(suggestions_area).on("mouseout", "span", function () {
      ...
      suggestions_area
        .find("span")
        .removeClass("sb-suggestion-full");
    });
      ...
      function () {
        ...
        switch (value) {
          case "delete":
            if (SBChat.user_online) {
              SBF.ajax(
                ...
                () => {
                  SBChat.conversation.deleteMessage(
                    message_id
                  );
                  message.remove();
                }
          ...
          case "original":
            ...
            let original_payload = keys
              .map((key) => message_.payload(key))
              .concat(message_.details.message);
            message_.set(
              "message",
              is_translation
                ? message_.payload("translation") ||
                    message_.get("message")
                : message_.payload("original-message") ||
                    message_.get("message")
            );
            keys.forEach(
              (key) => delete message_.details.payload[key]
            );
            message.replaceWith(
              message_
                .getCode()
            ...
            keys.forEach((key, i) =>
              message_.payload(key, original_payload[i])
            );
          ...
          case "reply":
            ...
            if (!text) {
              message_.attachments.forEach((attachment) => {
                text +=
                  '<i class="sb-icon-clip"></i>' +
                  attachment[0];
              });
            }
            conversations_area
              .find(".sb-editor [data-reply]")
              .remove();
            conversations_area
              .find(".sb-editor")
              .prepend(
                `<div data-reply="${message_id}"${
                  is_agent ? ' class="sb-reply-color"' : ""
                }><span>${
                  (is_agent && admin) ||
                  (!is_agent && !admin)
                    ? sb_("You")
                    : message_.get("full_name")
                }</span>${text}<i class="sb-icon-close" onclick="SBChat.cancelReply()"></i></div>`
              );
            conversations_area_list.addClass(
              "sb-reply-active"
            );
            break;
        }
    ...
    // Attachments filter
    attachments_panel.on(
      "click",
      "#sb-attachments-filter li",
      function () {
        let links = attachments_panel.find(
          "a:not(.sb-collapse-btn)"
        );
        ...
        links.each(function () {
          $(this).setClass(
            "sb-hide",
            file_type &&
              SBF.getFileType($(this).attr("href")) !=
                file_type
          );
        });
        collapse(attachments_panel, 160);
    ...
    $(admin).on(
      "click",
      ".sb-cc-box #sb-save-changes",
      function () {
        let cc = admin
          .find(".sb-cc-box .repeater-item input")
          .map(function () {
            return $(this).val();
          })
          .get()
    ...
    $(users_table).on(
      ...
      "th:not(:first-child)",
      function () {
        let direction = $(this).hasClass("sb-order-asc")
          ? "DESC"
          : "ASC";
    ...
    $(users_area).on("click", ".sb-new-user", function () {
      ...
      profile_edit_box
        .find(".sb-top-bar .sb-save")
        .html(
          `<i class="sb-icon-check"></i>${sb_("Add user")}`
        );
      profile_edit_box
        .find("input,select,textara")
        .removeClass("sb-error");
    ...
    $(profile_edit_box).on(
      ...
      function () {
        if (loading(this)) return;
        let new_user = profile_edit_box.hasClass(
          "sb-user-new"
        )
          ? true
          : false;
        ...
        // Get settings
        let settings = SBProfile.getAll(
          profile_edit_box.find(".sb-details")
        );
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            activeUser().update(() => {
              ...
              infoBottom(
                new_user ? "New user added" : "User updated"
              );
            });
    ...
    $(admin).on(
      ...
      function () {
        ...
        if (whatsapp) {
          let select = box.find(
            "#sb-whatsapp-send-template-list"
          );
          template_name = select.val();
          template_languages = select
            .find("option:selected")
            .attr("data-languages");
          phone_number_id = select
            .find("option:selected")
            .attr("data-phone-id");
          parameters = ["header", "body", "button"].map(
            (id) =>
              box
                .find(`#sb-whatsapp-send-template-${id}`)
                .val()
          );
        }
        if (SBForm.errors(box)) {
          ...
          if (
            ...
            parameters.join("").includes("recipient_name")
          ) {
            user_details.push("first_name", "last_name");
          }
          if (
            message.includes("recipient_email") ||
            parameters.join("").includes("recipient_email")
          ) {
          ...
          if (type == "message") {
            SBF.ajax(
              ...
              (response) => {
                $(this).sbLoading(false);
                let send_email =
                  SB_ADMIN_SETTINGS.notify_user_email;
                let send_sms =
                  SB_ADMIN_SETTINGS.sms_active_users;
                if (SBF.errorValidation(response)) {
      ...
      SBF.ajax(
        {
          ...
          user_name: user_ids.first_name
            ? (
                user_ids.first_name[i].value +
                " " +
                user_ids.last_name[i].value
              ).trim()
            : false,
          user_email: user_ids.email
            ? user_ids.email[i].value
            : false,
        },
        (response) => {
            ...
            if (i < user_ids_length - 1) {
              ...
              infoBottom(
                user_ids_length == 1
                  ...
                  : sb_(
                      "The message was sent to all users who have" +
                        settings[2]
                    )
              );
            }
    ...
    $(admin).on(
      "click",
      '.sb-repeater-upload, [data-type="upload-image"] .image, [data-type="upload-file"] .sb-btn, #sb-chatbot-add-files, #sb-import-settings a, #sb-import-users a',
      function () {
        let extensions = "";
        upload_target = this;
        if ($(this).attr("id") == "sb-chatbot-add-files") {
          ...
          upload_function = function () {
            ...
            for (var i = 0; i < files.length; i++) {
              ...
              code += `<tr class="sb-pending" data-name="${
                files[i].name
              }"><td><input type="checkbox" /></td><td>${
                files[i].name
              }<label>${sb_("Pending")}</label></td><td>${
                size ? size : 1
              } KB</td><td><i class="sb-icon-delete"></i></td></tr>`;
            }
          ...
          upload_on_success = (response) => {
            ...
            if (
              parent
                .find(".repeater-item:last-child input")
                .val()
            ) {
    ...
    $(settings_area)
      ...
      .focusout(function () {
        ...
        if (
          ...
          ["color-admin-1", "color-1"].includes(
            $(this).attr("id")
          )
        ) {
          color = "";
    ...
    $(settings_area).on(
      "change",
      '#saved-replies [data-id="reply-name"], [data-id="rich-message-name"]',
      function () {
        $(this).val(
          $(this)
            .val()
    ...
    $(settings_area).on(
      "change",
      '#user-additional-fields [data-id="extra-field-name"]',
      function () {
        $(this)
          .parent()
          .next()
          .find("input")
          .val(SBF.stringToSlug($(this).val()));
      }
    );
        ...
        if (
          ...
          if (SB_ADMIN_SETTINGS.credits) {
            window.open(
              url.replace(
                "{client_id}",
                SB_ADMIN_SETTINGS.google_client_id
              )
            );
            e.preventDefault();
            return false;
          ...
          let client_id = settings_area
            .find("#google-client-id input")
            .val();
          if (
            client_id &&
            settings_area
              .find("#google-client-secret input")
              .val()
          ) {
            window.open(
    ...
    $(settings_area).on(
      ...
      function (e) {
        infoPanel(
          `<pre>${SB_URL}/apps/dialogflow/functions.php</pre>`
        );
        e.preventDefault();
        return false;
      }
    ...
    $(settings_area).on(
      "click",
      "#email-server-troubleshoot a",
      function (e) {
        settings_area
          .find("#test-email-user input")
          .val(SB_ACTIVE_AGENT.email)
          .next()
          .click()[0]
    ...
    $(settings_area).on(
      ...
      function () {
        ...
        if (active) {
          $(this)
            .sbActive(false)
            .find(".sb-custom-select")
            .remove();
        } else {
          let select = $(timetable)
            .find("> .sb-custom-select")
            .html();
          $(timetable)
            .find(" > div .sb-custom-select")
            .remove();
          $(this)
            .append(
              `<div class="sb-custom-select">${select}</div>`
            )
            .sbActive(true);
        }
      }
    ...
    $(settings_area).on(
      "click",
      "#push-notifications-btn a",
      function (e) {
        if (
          SB_ADMIN_SETTINGS.cloud ||
          settings_area
            .find("#push-notifications-provider select")
            .val() == "onesignal"
        ) {
    ...
    $(settings_area).on(
      ...
      function () {
        ...
        SBF.search(search, () => {
          let code = "";
          let dropdown = settings_area.find(
    ...
    $(settings_area).on(
      "click",
      ".sb-search-dropdown-items div",
      function () {
        let index = $(this).attr("data-tab-index");
        let item = settings_area
          .find("> .sb-tab > .sb-nav li")
          .eq(index);
        item.click();
        ...
        SBF.ajax(
          {
            ...
            attachments: [
              ...
              [
                "Example link two",
                SB_URL + "/media/user.svg",
              ],
            ],
            channel: settings_area
              .find("#slack-channel input")
              .val(),
          },
          (response) => {
            if (SBF.errorValidation(response)) {
      ...
      function (e) {
        ...
        SBF.ajax(
          ...
          (response) => {
            if (response === true) {
              infoPanel(
                "Slack channels archived successfully!"
              );
            }
            $(this).sbLoading(false);
          }
    ...
    $(settings_area).on(
      "click",
      '#whatsapp-twilio-btn a, #whatsapp-twilio-get-configuartion-btn a, #sms-btn a, #wechat-btn a, #twitter-callback a, #viber-webhook a, #zalo-webhook a, [data-id="line-webhook"], #messenger-path-btn a',
      function (e) {
    ...
    $(settings_area).on(
      ...
      function (e) {
        ...
        if (!id) {
          let buttons = {
            "telegram-numbers-button": "telegram-button",
          };
          let inputs = {
            "telegram-button": "telegram-numbers-token",
          };
        ...
        SBF.ajax(
          ...
          (response) => {
            infoPanel(
              calls[2][0] in response &&
                response[calls[2][0]] == calls[2][1]
          ...
          SBF.ajax(
            ...
            (response) => {
              ...
              if (response) {
                ...
                settings_area
                  .find(
                    "#messenger-pages .repeater-item > i"
                  )
                  .click();
                setTimeout(() => {
                  SBSettings.save();
      ...
      function (e) {
        ...
        SBApps.wordpress.ajax("wp-sync", {}, (response) => {
          if (response === true || response === "1") {
            SBUsers.update();
            infoPanel(
              "WordPress users successfully imported."
            );
          } else {
            infoPanel(
    ...
    $(settings_area).on(
      ...
      function () {
        settings_area
          .find(
            `.sb-translations [data-area="${$(this).data(
              "value"
            )}"]`
          )
          .sbActive(true)
    ...
    $(settings_area).on(
      ...
      function () {
        settings_area
          ...
          .prepend(
            `<div class="sb-setting sb-type-text sb-new-translation"><input type="text" placeholder="${sb_(
              "Enter original text..."
      ...
      function () {
        ...
        SBF.search(search, () => {
          if (search.length > 1) {
            settings_area
              .find(
                ".sb-translations .sb-content > .sb-active label"
              )
              .each(function () {
                ...
                if (
                  ...
                  let scroll_area = settings_area.find(
                    ".sb-scroll-area"
                  );
                  scroll_area[0].scrollTop = 0;
                  scroll_area[0].scrollTop =
                    $(this).position().top - 80;
                  temp = value;
                  return false;
                }
              });
    ...
    $(admin).on(
      ...
      function () {
        $(this)
          ...
          .setClass(
            "sb-hide",
            ["is-set", "is-not-set"].includes($(this).val())
          );
      }
    );

    $(automations_area_select).on(
      "click",
      "li",
      function () {
        SBSettings.automations.populate(
      ...
      function (e) {
        ...
        switch (id) {
          ...
          case "close-message":
            if (
              settings_area
                .find("#close-active input")
                .is(":checked") &&
              settings_area
                .find("#close-chat input")
                .is(":checked")
            ) {
              infoBottom(
                warning_messages[0]
          ...
          case "routing":
            if (
              ((id == "queue" &&
                ...
                (id == "follow-message" &&
                  settings_area
                    .find("#follow-active input")
                    .is(":checked")) ||
                (id == "chat-timetable" &&
                  settings_area
                    .find("#chat-timetable-active input")
                    .is(":checked"))) &&
              settings_area
                .find(
                  "#dialogflow-human-takeover-active input"
                )
                .is(":checked")
            ) {
              infoBottom(
          ...
          case "sms":
            if (
              ((id == "sms" &&
                settings_area
                  .find("#sms-active-agents input")
                  .is(":checked")) ||
                (id == "push-notifications" &&
                  settings_area
                    .find(
                      "#push-notifications-active input"
                    )
                    .is(":checked")) ||
                (id == "notify-agent-email" &&
                  settings_area
                    .find("#notify-agent-email input")
                    .is(":checked"))) &&
              settings_area
                .find(
                  "#dialogflow-human-takeover-active input"
          ...
          case "privacy":
            if (
              settings_area
                .find("#privacy-active input")
                .is(":checked") &&
              settings_area
                .find("#registration-required select")
                .val()
            ) {
              infoBottom(
                warning_messages[0]
          ...
          case "google-multilingual-translation":
            if (
              $(e.target).is(":checked") &&
              settings_area
                .find("#open-ai-active input")
                .is(":checked") &&
              !settings_area
                .find(
                  "#open-ai-training-data-language select"
                )
                .val()
            ) {
              infoBottom(
          ...
          case "open-ai-prompt":
            infoBottom(
              "Custom prompts may break the chatbot, we advise leaving it empty.",
              "info"
            );
            break;
          case "open-ai-tokens":
          ...
          case "open-ai-custom-model":
            infoBottom(
              "This is an advanced setting, incorrect values may break the chatbot. If you're unsure, leave it empty.",
              "info"
            );
            break;
          case "open-ai-user-train-conversations":
            if ($(e.target).is(":checked")) {
              infoBottom(
                "This method is not recommended. See the docs for details.",
                "info"
              );
            }
            break;
    ...
    $(users_table_menu).on(
      ...
      function () {
        if (!SB_ADMIN_SETTINGS.visitors_registration) {
          infoBottom(
            "Settings > Users > Register all visitors must be enabled to see online users.",
            "info"
          );
        }
      }
    ...
    $(settings_area).on(
      "click",
      "#dialogflow-active,#dialogflow-welcome,#dialogflow-departments",
      function (e) {
        // Depreceated
        infoBottom(
          "Warning! We will stop supporting Dialogflow by the end of 2025. All its features will be available in Support Board through OpenAI. Please use OpenAI instead of Dialogflow.",
          "info"
        );
      }
    );
    ...
    $(settings_area).on(
      "change",
      "#registration-required select",
      function () {
        let value = $(this).val();
        if (
          ["registration-login"].includes(value) &&
          !settings_area
            .find('[id="reg-email"] input')
            .is(":checked")
        ) {
          infoBottom(
        ...
        if (
          ...
          infoBottom(
            warning_messages[0]
              .replace("{R}", "Messages > Privacy message")
              .replace(
                "{R2}",
                "Users > Require registration"
              ),
    ...
    $(chatbot_area).on(
      ...
      function () {
        SBAdmin.genericPanel(
          "flow-add",
          "Enter the flow name",
          '<div class="sb-setting"><input type="text"></div>',
          ["Add new flow"]
        );
      }
    );
    ...
    $(admin).on("click", "#sb-add-new-flow", function () {
      SBApps.openAI.flows.set(
        admin
          .find(".sb-flow-add-box input")
          .val()
          .replace(
            /[^a-zA-Z\u00C0-\u1FFF\u2C00-\uD7FF\uAC00-\uD7A3]/g,
            ""
    ...
    $(flows_area).on(
      ...
      function () {
        flows_area
          .find(".sb-flow-block,.sb-flow-add-block")
          .sbActive(false);
        $(this).sbActive(true);
        ...
        let code_2 = "";
        let type = $(this).attr("data-type");
        let code_repeater = `<div class="sb-title">{R}</div><div data-type="repeater" class="sb-setting sb-type-repeater"><div class="input"><div class="sb-repeater">{R2}</div><div class="sb-btn sb-btn-white sb-repeater-add sb-icon"><i class="sb-icon-plus"></i>${sb_(
          "Add new item"
        )}</div></div></div>`;
        let code_conditions = `<div class="sb-title">${sb_(
          "Conditions"
        )}</div><div class="sb-flow-conditions"></div><div class="sb-add-condition sb-btn sb-icon sb-btn-white"><i class="sb-icon-plus"></i>${sb_(
          "Add condition"
        )}</div>`;
        let code_message = `<div class="sb-title">${sb_(
          "Message"
        )}</div><div class="sb-setting"><textarea placeholder="${sb_(
          "The message sent to the user..."
        )}">${block.message}</textarea></div>`;
        let code_select_user_details =
          SBApps.openAI.getCode.select_user_details();
        switch (type) {
          case "start":
            code = `<div class="sb-title">${sb_(
              "Start event"
            )}</div><div class="sb-setting"><select class="sb-flow-start-select"><option value="message"${
              block.start == "message" ? " selected" : ""
            }>${sb_(
              "User message"
            )}</option><option value="conversation"${
              block.start == "conversation"
                ? " selected"
                : ""
            }>${sb_(
              "New conversation started"
            )}</option><option value="load"${
              block.start == "load" ? " selected" : ""
            }>${sb_(
              "On page load"
            )}</option></select></div><div class="sb-title sb-title-flow-start${
              block.start == "message" ? `` : ` sb-hide`
            }">${sb_(
              "User message"
            )}</div><div data-type="repeater" class="sb-setting sb-flow-start-messages sb-type-repeater"><div class="input"><div class="sb-repeater"><div class="repeater-item"><div class="sb-setting"><textarea data-id="message"></textarea></div><i class="sb-icon-close"></i></div></div><div class="sb-btn sb-btn-white sb-repeater-add sb-icon"><i class="sb-icon-plus"></i>${sb_(
              "Add message"
            )}</div></div></div>${code_conditions}<div class="sb-title">${sb_(
              "Disabled"
            )}</div><div class="sb-setting"><input type="checkbox" id="sb-flow-disabled"${
              block.disabled ? " checked" : ""
            }></div>`;
            break;
          case "button_list":
          ...
          case "message":
            ...
            for (
              var i = 0;
              i < block.attachments.length;
              i++
            ) {
              code_2 += `<div class="repeater-item"><div><input data-id type="text" value="${
                block.attachments[i]
              }" placeholder="${sb_(
                "Enter a link..."
              )}"></div><i class="sb-icon-close"></i></div>`;
            ...
            code =
              code_message +
              code_repeater
                .replace(`{R}`, sb_("Attachments"))
                .replace(`{R2}`, code_2)
                .replace(
                  `</div></div></div>`,
                  `</div><i class="sb-repeater-upload sb-btn-icon sb-icon-clip"></i></div></div>`
                );
            break;
          case "video":
            code = `${code_message}<div class="sb-title">${sb_(
              "Video URL"
            )}</div><div class="sb-setting"><input type="url" placeholder="${sb_(
          ...
          case "get_user_details":
            ...
            for (var i = 0; i < block.details.length; i++) {
              code_2 += `<div class="repeater-item"><div>${code_select_user_details.replace(
                `"${block.details[i][0]}"`,
                `"${block.details[i][0]}" selected`
              )}<div class="sb-setting"><input type="text" placeholder="${sb_(
                "Enter a description..."
              )}" value="${
                block.details[i][1]
              }" /></div><div class="sb-setting"><label>${sb_(
                "Required"
              )}</label><input type="checkbox" ${
                block.details[i][2] ? " checked" : ""
              }></div></div><i class="sb-icon-close"></i></div>`;
            }
            code =
              code_message +
              code_repeater
                .replace(`{R}`, sb_("User details"))
                .replace(`{R2}`, code_2)
                .replace(
                  "sb-type-repeater",
                  "sb-type-repeater sb-repeater-block-user-details"
                );
            break;
          case "set_data":
            code = SBApps.openAI.getCode.set_data(
              block.data
            );
            break;
          case "action":
            code = SBApps.openAI.getCode.actions(
              block.actions
            );
            break;
          case "rest_api":
            ...
            code = `<div class="sb-title">${sb_(
              "URL"
            )}</div><div class="sb-setting"><input type="url" class="sb-rest-api-url" value="${
              block.url
            }"></div><div class="sb-title">${sb_(
              "Method"
            )}</div><div class="sb-setting"><select class="sb-rest-api-method"><option value="GET"${
              block.method == "GET" ? " selected" : ""
            }>GET</option><option value="POST"${
              block.method == "POST" ? " selected" : ""
            }>POST</option><option value="PUT"${
              block.method == "PUT" ? " selected" : ""
            }>PUT</option><option value="PATH"${
              block.method == "PATH" ? " selected" : ""
            }>PATH</option><option value="DELETE"${
              block.method == "DELETE" ? " selected" : ""
            }>DELETE</option></select></div><div class="sb-title">${sb_(
              "Body"
            )}</div><div class="sb-setting"><textarea placeholder="JSON">${
              block.body
            }</textarea></div>`;
            for (var i = 0; i < keys.length; i++) {
              ...
              code += `<div class="sb-title">${sb_(
                SBF.slugToString(keys[i])
              )}</div><div data-type="repeater" class="sb-setting sb-type-repeater sb-repeater-block-rest-api sb-rest-api-${
                keys[i]
              }"><div class="input"><div class="sb-repeater">`;
              for (var j = 0; j < values.length; j++) {
                code += `<div class="repeater-item"><div>${
                  i == 1
                    ? code_select_user_details.replace(
                        `"${values[j][0]}"`,
                        `"${values[j][0]}" selected`
                      )
                    : `<div class="sb-setting"><input type="text" placeholder="${sb_(
                        "Key"
                      )}" value="${values[j][0]}" /></div>`
                }<div class="sb-setting"><input type="text" placeholder="${sb_(
                  i == 1 ? "e.g. data.id" : "Value"
                )}" value="${
                  values[j][1]
                }" /></div></div><i class="sb-icon-close"></i></div>`;
              }
        ...
        code += `<div id="sb-block-delete" class="sb-btn-text"><i class="sb-icon-delete"></i>${sb_(
          "Delete"
        )}</div>`;
        SBAdmin.genericPanel(
          "flow-block",
          SBF.slugToString(type),
          code,
          ["Save changes"],
          "",
          true
        ...
        if (type == "start" || type == "condition") {
          if (!Array.isArray(block.message))
            block.message = [{ message: block.message }]; // Depreceated
          let repeater = admin.find(
            ".sb-flow-start-messages .sb-repeater"
          );
          let code = SBSettings.repeater.set(
            block.message,
            repeater.find(".repeater-item:last-child")
          );
          SBSettings.automations.setConditions(
            block.conditions,
            admin.find(".sb-flow-conditions")
          );
          if (code) {
            repeater.html(code);
          }
        ...
        flows_area.find("> div").each(function () {
          step_scroll_positions.push(
            $(this).find("> div")[0].scrollTop
          );
        });
      }
    );
    ...
    $(admin).on(
      "click",
      ".sb-flow-block-box #sb-save-changes",
      function () {
        let block = SBApps.openAI.flows.blocks.get();
        ...
        switch (block.type) {
          case "start":
            block.message = SBSettings.repeater.get(
              box.find(
                ".sb-flow-start-messages .repeater-item"
              )
            );
            block.start = box.find("select").val();
            block.disabled = box
              .find("#sb-flow-disabled")
              .is(":checked");
            block.conditions =
              SBSettings.automations.getConditions(
                box.find(".sb-flow-conditions")
              );
            if (
              block.message.length &&
              block.message[0].message.trim().split(" ")
                .length < 3
            ) {
              return box
                .find(".sb-info")
          ...
          case "set_data":
            block[
              block.type == "action" ? "actions" : "data"
            ] = box
              .find(".repeater-item")
              .map(function () {
                return [
          ...
          case "rest_api":
            block.headers = box
              .find(".sb-rest-api-headers .repeater-item")
              .map(function () {
                return [
                  [
              ...
              .get();
            block.save_response = box
              .find(
                ".sb-rest-api-save_response .repeater-item"
              )
              .map(function () {
                return [
                  [
        ...
        flows_area.find("> div").each(function () {
          $(this).find("> div")[0].scrollTop =
            step_scroll_positions[$(this).index()];
        });
        admin.sbHideLightbox();
      }
    );
    ...
    $(admin).on(
      "change",
      ".sb-repeater-block-actions select",
      function () {
        ...
        $(this)
          ...
          .parent()
          .append(
            SBApps.openAI.getCode.action($(this).val(), "")
          );
      }
    );
    ...
    $(admin).on(
      "change",
      ".sb-flow-start-select",
      function () {
        admin
          .find(
            ".sb-title-flow-start, .sb-flow-start-messages"
          )
          .setClass("sb-hide", $(this).val() != "message");
      }
    );

    $(flows_area).on(
      "mouseleave",
      ".sb-flow-connectors > div, .sb-flow-block",
      function () {
        flows_area
          .find(".sb-flow-block-cnt")
          .sbActive(false);
        ...
        if (
          $(this).parent().hasClass("sb-flow-connectors")
        ) {
          SBApps.openAI.flows.blocks.activateLinkedCnts(
            $(this).closest(".sb-flow-block")
          );
        } else {
          flows_area
            .find(".sb-flow-connectors > div")
    ...
    $(flows_area).on(
      ...
      function () {
        let block_cnt = $(this)
          .closest(".sb-flow-block")
          .parent();
        let next_block_cnt_indexes =
          SBApps.openAI.flows.blocks.getNextCntIndexes(
            SBApps.openAI.flows.getActiveIndex(),
            block_cnt.parent().parent().index(),
            block_cnt.index()
          );
        is_over_connector = true;
        flows_area
          .find("> div")
          .eq(block_cnt.parent().parent().index() + 1)
          .find(".sb-flow-block-cnt")
          .sbActive(false)
          .eq(next_block_cnt_indexes[$(this).index()])
          .sbActive(true);
      }
    );
    ...
    $(flows_area).on(
      ...
      function () {
        flows_area
          ...
          .sbActive(false);
        $(this).sbActive(true);
        let active_blocks = SBApps.openAI.flows.steps
          .get()
          [
            SBApps.openAI.flows.blocks.getActiveCntIndex()
          ].map((item) => item.type);
        let all = !active_blocks.some((element) =>
          [
            "message",
            "button_list",
            "video",
        ...
        SBAdmin.genericPanel(
          ...
          `<ul class="sb-menu">${
            all
              ? `<li>${sb_(
                  "Messages"
                )} <ul><li data-value="message">${sb_(
                  "Send message"
                )}</li><li data-value="button_list">${sb_(
                  "Send button list"
                )}</li><li data-value="video">${sb_(
                  "Send video"
                )}</li></ul></li>`
              : ``
          }<li>${sb_("More")} <ul>${
            all
              ? `<li data-value="get_user_details">${sb_(
                  "Get user details"
                )}</li>`
              : ``
          }${code}</ul></li></ul>`
    ...
    $(admin).on(
      "click",
      ".sb-flows-blocks-nav-box [data-value]",
      function () {
        SBApps.openAI.flows.blocks.add(
          $(this).data("value")
        );
        admin.sbHideLightbox();
      }
    ...
    $(admin).on(
      ...
      ".sb-flow-scroll",
      function () {
        let is_back = $(this).hasClass(
          "sb-icon-arrow-left"
        );
        flow_scroll_interval = setInterval(() => {
          flows_area[0].scrollLeft +=
            10 * (is_back ? -1 : 1);
        }, 10);
      }
    );
    ...
    $(chatbot_area).on(
      ...
      function (e) {
        ...
        let nav = chatbot_area.find(
          '.sb-nav [data-value="conversations"]'
        );
        infoPanel(
          "<br><br><br><br><br>",
          "info",
        ...
        if (
          chatbot_area
            .find(".sb-menu-chatbot .sb-active")
            .attr("data-type") == "flows"
        ) {
        ...
        if (nav.sbActive()) {
          ...
          for (
            var i = 0;
            i < conversations_qea.length;
            i++
          ) {
            let qea = chatbot_area.find(
              `#sb-chatbot-conversations [data-index="${i}"]`
            );
            if (qea.length) {
              qea = [
                qea.find("input").val(),
                qea.find("textarea").val(),
              ];
              if (
                qea[0] !=
                  $("<textarea />")
                    .html(conversations_qea[i].question)
                    .text() ||
                qea[1] !=
                  $("<textarea />")
                    .html(conversations_qea[i].answer)
                    .text()
              ) {
          ...
          if (to_update.length) {
            SBF.ajax(
              {
                function:
                  "open-ai-save-conversation-embeddings",
                qea: to_update,
              },
              (response) => {
                if (response[0] === true) {
                  ...
                  !SBApps.openAI.train.isError(response[0])
                ) {
        ...
        SBApps.openAI.train.files((response) => {
          // Website
          SBApps.openAI.train.urls = chatbot_area
            .find('[data-id="open-ai-sources-url"]')
            .map(function () {
              return $(this).val().trim();
            })
            .get();
          SBApps.openAI.train.extract_url = chatbot_area
            .find('[data-id="open-ai-sources-extract-url"]')
            .map(function () {
          ...
          SBApps.openAI.train.website((response) => {
            ...
            SBApps.openAI.train.qea((response) => {
              ...
              SBApps.openAI.train.articles((response) => {
                ...
                chatbot_area
                  .find(
                    "#sb-repeater-chatbot-website .repeater-item i"
                  )
                  .click();
                ...
                if (SBApps.openAI.train.errors.length) {
                  ...
                  !SBApps.openAI.train.isError(response)
                ) {
                  infoPanel(
                    success_text,
                    "info",
    ...
    $(chatbot_area).on(
      "click",
      "#sb-table-chatbot-files td i, #sb-table-chatbot-website td i, #sb-chatbot-delete-files, #sb-chatbot-delete-website, #sb-chatbot-delete-all-training, #sb-chatbot-delete-all-training-conversations",
      function () {
        let is_i = $(this).is("i");
        let tr = is_i ? $(this).closest("tr") : false;
        if (is_i && tr.hasClass("sb-pending")) {
          SBApps.openAI.train.skip_files.push(
            tr.attr("data-name")
          );
          tr.remove();
          return;
        ...
        infoPanel(
          ...
          () => {
            ...
            if (is_i) {
              ...
              id == "sb-chatbot-delete-all-training"
            ) {
              sources_to_delete = "all";
            } else if (
              id ==
              "sb-chatbot-delete-all-training-conversations"
            ) {
              sources_to_delete = "all-conversations";
            } else {
              let table = $(
                id == "sb-chatbot-delete-files"
                  ? chatbot_files_table
                  : chatbot_website_table
              );
              ...
              table.find("tr").each(function () {
                if ($(this).find("input:checked").length) {
                  if ($(this).hasClass("sb-pending")) {
                    ...
                    if (
                      SBApps.openAI.train.sitemap_processed_urls.indexOf(
                        url
                      ) > -1
                    ) {
                      SBApps.openAI.train.sitemap_processed_urls[
                        SBApps.openAI.train.sitemap_processed_urls.indexOf(
                          url
                        )
                      ] = false;
    ...
    $(chatbot_area).on(
      ...
      '.sb-nav [data-value="conversations"]',
      function () {
        let area = chatbot_area.find(
          "#sb-chatbot-conversations"
        );
        ...
        SBF.ajax(
          ...
          (response) => {
            if (!response.length) {
              ...
              for (var i = 0; i < response.length; i++) {
                code += `<div class="repeater-item" data-value="${
                  response[i].id
                }" data-index=${i}><div><label>${sb_(
                  "Question"
                )}</label><input data-id="q" type="text" value="${
                  response[i].question
                }" /></div><div class="sb-qea-repeater-answer"><label>${sb_(
                  "Answer"
                )}</label><textarea data-id="a">${
                  response[i].answer
                }</textarea></div><i class="sb-icon-close"></i></div>`;
              }
      ...
      function () {
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            for (var i = 0; i < list.length; i++) {
              code += response[list[i][0]]
                ? `${response[list[i][0]][1]} ${sb_(
                    list[i][1]
                  )} (${response[list[i][0]][0]} ${sb_(
                    "chars"
                  )})<br>`
                : "";
            }
            code += `</p><h2>${sb_(
              ...
              (response.limit
                ? " / " +
                  ...
                  sb_("limit")
                : "")
            }</p><hr><div id="sb-chatbot-delete-all-training" class="sb-btn sb-btn-white">${sb_(
                ...
                (response) => {
                  for (
                    ...
                    if (
                      ...
                      SBApps.openAI.flows.flows.push(
                        response[i]
                      );
                    }
                  }
                  ...
                  for (
                    ...
                    if (response[i]) {
                      code += SBApps.openAI.flows.navCode(
                        response[i].name
                      );
                    }
                  }
    ...
    $(chatbot_qea_repeater).on(
      "change",
      '[data-id="open-ai-faq-set-data"] select',
      function () {
        $(this)
          .parent()
          .next()
    ...
    $(chatbot_qea_repeater).on(
      ...
      function () {
        let textarea = $(this)
          .closest(".repeater-item")
          .find('[data-id="open-ai-faq-answer"]');
        if (loading(this)) return;
    ...
    $(chatbot_playground_editor).on(
      "click",
      '[data-value="add"], [data-value="send"]',
      function () {
        let textarea =
          chatbot_playground_editor.find("textarea");
        let message = textarea.val().trim();
        textarea.val("");
        if (message) {
          SBApps.openAI.playground.addMessage(
            message,
            chatbot_playground_editor
              .find(
                '[data-value="user"], [data-value="assistant"]'
              )
              .attr("data-value")
          );
        }
        if ($(this).data("value") == "send") {
          let length =
            SBApps.openAI.playground.messages.length;
              ...
              (response) => {
                if (response[0]) {
                      ...
                      for (var key in response[4].usage) {
                        if (
                          ["string", "number"].includes(
                            typeof response[4].usage[key]
                          )
                        ) {
                          code += `<b>${SBF.slugToString(
                            key
                          )}</b>: ${
                            response[4].usage[key]
                          }<br>`;
                        }
                      }
                      SBApps.openAI.playground.last_response =
                        response[4];
                      chatbot_area
                        .find(".sb-playground-info")
                        .html(
                          code +
                            `<div id="sb-playground-query" class="sb-btn-text">${sb_(
                              "View code"
                            )}</div>${
                              response[4].embeddings
                                ? `<div id="sb-playground-embeddings" class="sb-btn-text">${sb_(
                                    "Embeddings"
                                  )}</div>`
                                : ``
                            }`
                  ...
                  if (response[8]) {
                    conversations_admin_list_ul
                      .find(
                        `[data-conversation-id="${response[8]}"]`
                      )
                      .remove();
                  }
                } else {
    ...
    $(chatbot_area).on(
      ...
      function () {
        infoPanel(
          "<pre>" +
            JSON.stringify(
              SBApps.openAI.playground.last_response.query,
              null,
              4
            ).replaceAll('\\"', '"') +
            "</pre>",
    ...
    $(chatbot_area).on(
      "click",
      "#sb-playground-embeddings",
      function () {
        let code = "";
        let embeddings =
          SBApps.openAI.playground.last_response.embeddings;
        for (var i = embeddings.length - 1; i > -1; i--) {
          code += `<span><b>${sb_("Source")}</b>: ${
            embeddings[i].source
              ? embeddings[i].source.autoLink({
                  target: "_blank",
                })
              : ""
          }<br><b>${sb_("Score")}</b>: ${
            embeddings[i].score
          }<br><span>${embeddings[i].text}</span></span>`;
        }
        infoPanel(
          code,
          "info",
          false,
          "sb-playground-embeddings-panel",
          false,
          true
        );
      }
    ...
    $(chatbot_playground_area).on(
      "click",
      ".sb-icon-close",
      function () {
        let element = $(this).closest("[data-type]");
        SBApps.openAI.playground.messages.splice(
          element.index(),
          1
        );
        element.remove();
    ...
    $(chatbot_playground_editor).on(
      ...
      function () {
        ...
        $(this)
          .attr(
            "data-value",
            is_user ? "assistant" : "user"
          )
          .html(
            '<i class="sb-icon-reload"></i> ' +
              sb_(is_user ? "Assistant" : "User")
          );
      }
    );
    ...
    $(admin).on(
      "click",
      "#open-ai-troubleshoot a, #google-troubleshoot a",
      function (e) {
        let id = $(this).parent().attr("id");
        e.preventDefault();
        if (
        ...
        SBF.ajax(
          ...
          (response) => {
            ...
            $(conversations_area)
              .find(
                ".sb-admin-list .sb-select li.sb-active"
              )
              .click();
          }
        );
    ...
    $(articles_area).on(
      ...
      function () {
        let url = articles_area
          .find(".image-tool--filled img")
          .attr("src");
        if (url) {
      ...
      function (e) {
        infoPanel(
          ...
          () => {
            ...
            articles_area
              .find(".sb-scroll-area:not(.sb-nav)")
              .scrollTop(0);
          },
          false,
          false,
      ...
      infoPanel(
        ...
        () => {
          ...
          if (is_category) {
            ...
            articles_area
              .find("#editorjs .image-tool__image-picture")
              .each(function () {
                SBF.ajax({
                  function: "delete-file",
                  path: $(this).attr("src"),
    ...
    $(articles_area).on(
      ...
      function () {
        if (loading(this)) return;
        if (
          articles_area.attr("data-type") == "categories"
        ) {
    ...
    $(reports_area).on(
      ...
      function () {
        ...
        reports_area.attr(
          "class",
          "sb-area-reports sb-active sb-report-" + id
        );
    ...
    $(conversations_area).on(
      ...
      function (e) {
        ...
        if (!parent.sbActive()) {
          SBApps.woocommerce.conversationPanelOrder(
            parent.attr("data-id")
          );
        }
      }
    ...
    $(conversations_area).on(
      ...
      function () {
        if (
          ...
          (activeUser() != false &&
            activeUser().language !=
              SBApps.itemsPanel.panel_language)
        ) {
    ...
    $(woocommerce_products_box).on(
      ...
      function () {
        let action =
          woocommerce_products_box.attr("data-action");
        let id = $(this).data("id");
        if (SBF.null(action)) {
          ...
          SBChat.sendMessage(
            ...
            (response) => {
              if (response) {
                SBApps.woocommerce.conversationPanelUpdate(
                  id
                );
                admin.sbHideLightbox();
              }
    ...
    $(conversations_area).on(
      ...
      function () {
        ...
        if (SBChat.user_online) {
          SBApps.itemsPanel.populate("woocommerce");
          woocommerce_products_box.sbShowLightbox(
            true,
            "cart-add"
          );
        } else {
      ...
      function () {
        ...
        switch (item.parent().attr("data-source")) {
          ...
          case "automations":
            ...
            break;
          case "settings":
            let active_language = item
              .parent()
              .find("[data-language].sb-active");
            ...
            SBSettings.translations.activate(
              item,
              language
            );
            break;
      ...
      function () {
        ...
        infoPanel(
          ...
          () => {
            switch (item.parent().attr("data-source")) {
              case "article-categories":
                SBArticles.categories.translations.delete(
                  language
                );
                break;
              case "articles":
                ...
                break;
              case "automations":
                SBSettings.automations.deleteTranslation(
                  false,
                  language
                );
                ...
                break;
              case "settings":
                SBSettings.translations.delete(
                  item,
                  language
                );
                break;
    ...
    $(admin).on(
      "click",
      ".sb-languages-box [data-language]",
      function () {
        ...
        switch (box.attr("data-source")) {
          case "article-categories":
            SBArticles.categories.translations.add(
              language
            );
            break;
          case "articles":
            ...
            break;
          case "automations":
            SBSettings.automations.addTranslation(
              false,
              false,
              language
            );
    ...
    $(admin).on(
      "click",
      ".sb-lightbox .sb-top-bar .sb-close",
      function () {
        admin.sbHideLightbox();
      }
    );
    ...
    $(admin).on("click", ".sb-dialog-box a", function () {
      ...
      if (
        admin.find(".sb-lightbox.sb-active").length == 1
      ) {
        overlay.sbActive(false);
      }
      SBAdmin.open_popup = false;
    ...
    $(admin).on(
      ...
      function () {
        ...
        hide.hide();
        SBAdmin.open_popup =
          show.hasClass("sb-lightbox") ||
          show.hasClass("sb-popup")
            ? show
            : false;
      }
    );
    ...
    $(upload_input).on("change", function () {
      if (upload_function) {
        ...
        $(upload_target).sbLoading(
          $(this).prop("files").length
        );
        $(this).sbUploadFiles((response) => {
          $(upload_target).sbLoading(false);
          response = JSON.parse(response);
          if (response[0] == "success") {
            let type = $(upload_target)
              .closest("[data-type]")
              .data("type");
      ...
      if ($(SBAdmin.open_popup).length) {
        ...
        if (
          ...
          if (popup.hasClass("sb-popup")) {
            ...
            !SBAdmin.open_popup ||
            !["sb-embeddings-box"].includes(
              SBAdmin.open_popup.attr("id")
            )
          ) {
            admin.sbHideLightbox();
          }
...
!(function (a, b) {
  ...
  function d(a, b, c) {
    ...
    return (
      ...
      o.options.customBG &&
        ...
        (e.rgbaMixCustom.luminance = h(
          e.rgbaMixCustom,
          !0
        )),
        (o.options.customBG.luminance = h(
          o.options.customBG,
          !0
        ))),
      e
  ...
  function e(a, b) {
    ...
    return (
      ...
      s.customBG &&
        ...
        (D.luminanceDelta = m.abs(
          D[G] - H.rgbaMixCustom[G]
        )),
  ...
  var l = {
    ...
    t = function (a, d) {
      ...
      (e = f.customBG),
        (f.customBG =
          "string" == typeof e ? v.txt2color(e).rgb : e),
    ...
    (s.prototype.setCustomBackground = function (a) {
      return (
        ...
        (this.options.customBG =
          "string" == typeof a ? v.txt2color(a).rgb : a),
        c(this.colors, b, "rgb")
      );
    }),
    ...
    HEX2rgb: function (a) {
      return (
        (a = a.split("")),
        {
          ...
          g:
            +("0x" + a[a[3] ? 2 : 1] + (a[3] || a[1])) /
            255,
          b:
            +("0x" + (a[4] || a[2]) + (a[5] || a[2])) / 255,
        }
      );
    },
    hue2RGB: function (a) {
      ...
      c
        ? ((s = g(d)),
          ...
          (t || i())
            ...
            .on(
              D,
              ".cp-xy-slider,.cp-z-slider,.cp-alpha",
              j
            ))
        : p.$trigger &&
          b(t)
    ...
    function n(a) {
      ...
      (v._css = {
        ...
        (s._css = {
          ...
          color: t
            ? ""
            : b.rgbaMixBGMixCustom.luminance > 0.22
            ? e
            : f,
        }),
        (s.text = t ? "" : p !== g ? g : ""),
        ...
        return (
          ...
          b(c.body)
            ...
            .on(D, function (a) {
              -1 ===
                B.add(t)
                  .add(b(t).find(a.target))
                  .index(a.target) && h();
            }),
          this.on("focusin.tcp click.tcp", function (a) {
            (p.color.options = b.extend(
              p.color.options,
              (r = d.options)
            )),
              h.call(this, a);
            ...
            .on("change.tcp", function () {
              q.setColor(this.value || "#FFF"),
                d.colorPicker.render(!0);
            })
            .each(function () {
              var a = e(this),
                d = a.split("("),
